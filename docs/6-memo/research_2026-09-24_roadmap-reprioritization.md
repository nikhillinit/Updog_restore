...
| Semantic convergence spec and plan (tracked, #1541)                                           | Items 1 to 5 (P0 defects), item 6 production release, items 7 to 10 product work                                                                  | Items 1 to 5 landed (#1544, #1548, #1543, #1549, #1542). Item 6 not done. Plan items 7 to 13 not scoped; the plan numbers them differently from the spec, and items 10, 11 and 13 are fabricated-value defects, not product work.                                            |
...
### Untracked roadmap detail (added after owner request)

QA closure PR C, state of each item on `main`:

- C1 open: `client/src/pages/pipeline.tsx:439` deals query has no `enabled`
  guard on `fundId`.
- C2 open: `AddDealModal.tsx:131` and `AddCompanyDialog.tsx:89` post through
  `apiRequest` with no per-intent `Idempotency-Key`.
- C3 open: `ReallocationTab.tsx:25` hard-codes `currentVersion = 1`.
- C4 not verified: `client/src/components/performance/PerformanceDashboard.tsx`
  exists; per-query error handling not checked.
- C5 open: `variance-tracking.tsx:1779` shows the acceptable-variance sentence
  based only on alert scope, not on a baseline or completed analysis.
- C6 open: `shared-dashboard.tsx` has no `aria-describedby` or label association
  for the passkey error.
- C7 open: no `tabIndex` on `ScenarioComparisonTable.tsx`.
- C8 unlocated: neither named service references the 0027 table; find where the
  raw query text reaches the UI before changing code.
- C9 open: `navigation-config.ts:147` enables only `dashboard` while
  `needsSetup` is true, so Help is disabled.
- C10 open: 24 files under `client/src/pages` still use the flagged gray and
  charcoal text classes (includes icon uses; sweep only readable text).

September 20 handoff durability (from the characterization test added in #1554):
`POST /api/funds/:id/recalculate` executes on every request and is not
database-backed; `POST /api/funds/calculate` deduplicates only in process, joins
a changed `fundSize` to the original calculation, and its `Location` returns 404
on `makeApp`. Only the recalculate route has a client caller.
