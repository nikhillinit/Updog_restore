...
A production-ready analytics system for the fund modeling platform, built from
...
### 5. **Cash Flow Generation** (`client/src/lib/cashflow/generate.ts`)
...
The analytics engine (XIRR, American waterfall ledger, Monte Carlo worker)
computes:

1. **Real-time IRR**: recomputed as fund parameters change
2. **Monte Carlo Analysis**: P10/P50/P90 scenarios with volatility
3. **Waterfall Preview**: DPI/TVPI with carry calculations
4. **Progress Tracking**: cancellable, chunked long-running calculations

> Note: the in-wizard `EnhancedAnalyticsPanel` UI was removed (2026-06-03,
> orphaned/unused). The engine modules above remain.
