...
- **Cardinality guards**: NEVER use `runId`, `userId`, or raw `scenario_id` in
  labels
- **Bucketing strategy**: Hash high-cardinality IDs into fixed buckets (0-63)

## SSE Streaming

Never implemented. The SSE stack was deleted and ADR-0003 deprecated on
2026-09-16 (ADR-102); the names below are kept for the record only. The SSE
dashboard row, PromQL queries, alert rule, and cardinality-budget row that this
document carried were removed the same day; nothing emits these series.
...
- `ai_token_budget_used_total{operation}` (counter) - Tokens consumed per
  operation type
...
- `ai_circuit_breaker_trips_total{service}` (counter) - Total trips to OPEN
  state
- `ai_circuit_breaker_half_open_probes_total{service}` (counter) - Recovery
  probe attempts

## Dead Letter Queue

- `ai_dlq_enqueued_total{reason}` (counter) - Jobs sent to DLQ by failure reason
- `ai_dlq_replayed_total{success}` (counter) - Replay attempts
  (success=true/false)
...
- `ai_evaluator_latency_milliseconds{scenario_bucket}` (histogram) - Evaluation
  duration
- `ai_evaluator_irr_delta{scenario_bucket}` (histogram) - Construction vs
  Current IRR delta
- `ai_evaluator_tvpi_delta{scenario_bucket}` (histogram) - Construction vs
  Current TVPI delta
...
```javascript
const labels = { scenario_id: 'abc-123-def-456' }; // BAD: unbounded cardinality
```
...
1. **Always use histograms for latencies** - Enables percentile queries without
   cardinality explosion
2. **Bucket scenario IDs** - Hash to fixed buckets (0-255) for attribution
   without unbounded labels
3. **Emit rates as counters** - Let Prometheus compute rates with `rate()` or
   `irate()`
4. **Label values must be bounded** - Whitelist allowed values, reject dynamic
   strings
