...
review_cadence: P90D
categories: [tech-debt, development, type-safety]
keywords: [typescript, any-elimination, type-guards, branded-types]
agent_routing:
  priority: 2
  use_cases: [task_execution]
related_code:
  - 'shared/units.ts'
...
**Created**: 2025-12-29 **Estimated Total Effort**: 40 hours **Priority**: P2
(Tech Debt)
...
Analysis of the codebase reveals **3,228 `any` usages across 742 files**,
significantly higher than initially estimated. However, the good news is that:

1. **Existing type infrastructure is underutilized** - branded types and type
   guards exist but are rarely imported
...
**Should be used in**: Monte Carlo engines, reserve calculators, performance
predictors
...
**Estimated Fixes**: ~177 `any` removed **Risk Level**: Low
...
**Estimated Fixes**: ~211 `any` removed **Risk Level**: Medium (requires
...
**Estimated Fixes**: ~201 `any` removed **Risk Level**: Medium
...
**Estimated Fixes**: ~200 `any` removed **Risk Level**: Low-Medium
