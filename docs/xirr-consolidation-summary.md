...
**Analysis Scope**: Complete XIRR implementation inventory across client and server code
...
**Note**: Implementations #5 and #6 are NOT true XIRR calculations - they are CAGR approximations for performance reasons.
...
**File**: `server/services/actual-metrics-calculator.ts`
**Issue**: Returns `0` on error instead of `null`
...
- `client/src/core/selectors/fund-kpis.ts` - Behavioral change (more robust fallback)
- `server/services/actual-metrics-calculator.ts` - Error handling change (0 → null)
...
**Options**:
A. `shared/financial/` - New package (RECOMMENDED)
B. `server/lib/financial/` - Server copy
C. `client/src/lib/finance/` - Make accessible to server
...
**Effort Estimate**: ~3-4 days (spread across 5 weeks for thorough testing)
...
- **Full Roadmap**: `/home/user/Updog_restore/docs/xirr-consolidation-roadmap.md`
- **Current Implementations**:
  ...
  - `/home/user/Updog_restore/server/services/actual-metrics-calculator.ts` (lines 173-212)
- **Key Test Suites**:
