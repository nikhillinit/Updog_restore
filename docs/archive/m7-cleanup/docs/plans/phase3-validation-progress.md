...
**Original Plan**: Docker pg-test container **Blocker Encountered**: Docker
Desktop not running **Resolution**: Switched to Neon cloud PostgreSQL (free
tier)
...
**Note**: Using `neondb` database instead of creating separate `updog_test`
...
**Discovery**: The `scripts/run-migrations.ts` has an ESM compatibility issue
(`require.main === module` doesn't work with tsx in ESM mode). Worked around by
executing migration logic inline via Node.js.
...
**Issue Found**: Multi-statement SQL files fail when sent as single query to
Neon PostgreSQL. The custom migration runner may need modification to split
statements.
...
**Indexes Created**: 9 new indexes (idx*job_outbox*_, idx*scenario_matrices*_,
idx*optimization_sessions*\*)
...
2. ESM compatibility issue in run-migrations.ts and rollback-migration.ts
   (require.main === module)
...
**Phase 5** (ScenarioMatrixCache): Requires Docker/Testcontainers - deferred to
CI **Phase 6** (Cleanup): N/A - used Neon cloud instead of Docker containers
...
**Reason**: Requires full browser environment with running dev server. **11
blocked tests** deferred to manual QA session. **Core functionality verified**
in Phase 7 unit tests (10/12 passed).
