...
**Date**: 2026-01-16 **Phase**: Discovery Pass
...
**Implication**: We don't need to manually start the server! The test setup does
it automatically.
...
2. **Use Docker**:
   `docker run -p 5432:5432 -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=povc_test postgres`
3. **Skip DB-dependent tests**: Not possible - setup.ts spawns server which
   requires DB
...
**Time Investment**: 5 minutes **Learning Value**: Confirm test 2 works, see
actual errors for tests 1 & 3
...
Note: This will still try to start the server (setup.ts runs), but test itself
doesn't use it.
...
1. [DONE] **Decision Made**: PostgreSQL is required for full validation
2. [NEXT] **Next**: Decide whether to invest in PostgreSQL setup (Option B)
3. [ALT] **Alternative**: Run reserves test only, defer others
