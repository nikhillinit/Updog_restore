...
**Date**: 2026-01-16 **Session**: Validation attempt
...
This setup file runs `beforeAll` for **all integration tests**, including our
reserves test.
...
**Global setup.ts runs for ALL integration tests**, even tests that don't need
the server.
...
const { calculateReservesSafe } =
  await import('./client/src/lib/reserves-v11.js');
...
This proves our dynamic import fix works without needing full test
infrastructure.
