...
4. **What have I learned?** Seeded PRNG solved Monte Carlo flakiness; Brent's
   method fixed XIRR edge cases
5. **What have I done?** Completed 20/25 tasks, 0 test failures, full
   documentation
...
- [x] Moved `tests/middleware/idempotency-dedupe.test.ts` to
...
2. Tests in `tests/middleware/` not in vitest paths - moved to
   `tests/unit/middleware/`
3. Header name mismatch: tests expected `x-idempotent-replay`, impl used
   `Idempotency-Replay`
...
- [x] Tasks 11-16: Fixed all E2E tests (fund setup, navigation, performance,
