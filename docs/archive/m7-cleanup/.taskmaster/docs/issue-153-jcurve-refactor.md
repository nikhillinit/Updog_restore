...
4. **NAV/DPI materialization** (lines 158-166): Delegate to
   `materializeNAVandDPI`
...
1. **API Contract**: Should we fix tests to match implementation, or update
   implementation to match test expectations?
2. **Breaking Changes**: Are there other consumers of `computeJCurvePath` that
   would break?
3. **Test Coverage**: Can tests run with current implementation? (They use
   different types)
