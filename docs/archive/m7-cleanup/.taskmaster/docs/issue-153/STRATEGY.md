...
Refactor `computeJCurvePath` using a **hybrid approach** that extracts 3 focused
helpers while leveraging existing modules (`fitTVPI`, `gompertz`, `logistic`).
...
2. **Create new golden tests** in `tests/unit/jcurve-refactor.test.ts` using
   actual API
