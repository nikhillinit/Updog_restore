...
1. **Percentile Hit Rate Logic ERROR**: Current p75 check uses 90% CI range
   (p5-p95). Must rename to p90 or compute proper 75% CI with p12.5/p87.5.

2. **Division by Zero Risk**:
   `normalizedError = error / Math.abs(actualValue || 1)` is a hack. Must
   properly handle null/zero actual values.

3. **Missing Authorization**: All endpoints need `requireAuth` +
   `validateFundAccess` middleware to prevent cross-fund data access.

### PRIORITY 1 - High Impact Additions

4. **Real Fund Data Integration (P0)**: The `getActualPerformance()` method MUST
   query actual fund baselines and variance reports, not return mock data.

5. **Market Parameter Application (P0)**: The `runScenarioComparisons()` method
   MUST modify the simulation config to use scenario-specific market parameters.

6. **Database Persistence (P1)**: Add a `backtest_results` table to persist
   history.

7. **Async Job Queue (P1)**: Add `/run/async` endpoint for large backtests (>10k
   runs).

8. **Rate Limiting**: Add rate limiter (5 backtests/minute per fund) to prevent
   DoS.

9. **Variance Tracking Integration**: Store backtest deviations in existing
   variance tracking system for unified reporting.

10. **Snapshot Versioning**: Link backtests to fund snapshots via new versioning
    service.
...
12. **Data Quality Checks**: Validate baseline freshness, variance history
    availability.
...
// Metric types
export const BacktestMetricSchema = z.enum([
  'irr',
  'tvpi',
  'dpi',
  'multiple',
...
export const BacktestConfigSchema = z
  .object({
    ...
    comparisonMetrics: z
      .array(BacktestMetricSchema)
      .min(1)
      .default(['irr', 'tvpi', 'dpi']),
...
> = {
  financial_crisis_2008: {
    ...
    endDate: '2009-12-31',
    description:
      'Global financial crisis with severe credit contraction, market volatility, and reduced exit opportunities',
    marketParameters: {
  ...
  dotcom_bust_2000: {
    ...
    endDate: '2002-12-31',
    description:
      'Dot-com bubble burst with tech sector collapse and extended recovery period',
    marketParameters: {
  ...
  covid_2020: {
    ...
    endDate: '2020-12-31',
    description:
      'COVID-19 pandemic with initial market shock followed by rapid tech-driven recovery',
    marketParameters: {
  ...
  bull_market_2021: {
    ...
    endDate: '2021-12-31',
    description:
      'Exceptional bull market with record valuations, SPACs, and abundant liquidity',
    marketParameters: {
  ...
  rate_hikes_2022: {
    ...
    endDate: '2022-12-31',
    description:
      'Rising interest rate environment with valuation compression and reduced exit activity',
    marketParameters: {
...
describe('BacktestingService', () => {
  ...
  describe('runBacktest', () => {
    ...
    it('calculates validation metrics correctly', async () => {
      ...
      expect(result.validationMetrics.modelQualityScore).toBeGreaterThanOrEqual(
        0
      );
      expect(result.validationMetrics.modelQualityScore).toBeLessThanOrEqual(
        100
      );
  ...
  describe('compareScenarios', () => {
    it('compares multiple historical scenarios', async () => {
      ...
      expect(
        result.find((s) => s.scenario === 'financial_crisis_2008')
      ).toBeDefined();
      expect(
        result.find((s) => s.scenario === 'bull_market_2021')
      ).toBeDefined();
    });
  });
});
...
export class BacktestingService {
  ...
  async runBacktest(config: BacktestConfig): Promise<BacktestResult> {
    ...
    const simulationSummary = this.extractSimulationSummary(
      simulationResult,
      config
    );
    ...
    if (
      config.includeHistoricalScenarios &&
      config.historicalScenarios?.length
    ) {
  ...
  private async runSimulation(config: BacktestConfig) {
    const simulationConfig = {
      ...
      timeHorizonYears: this.calculateTimeHorizon(
        config.startDate,
        config.endDate
      ),
  ...
  private calculateValidationMetrics(
    ...
    for (const metric of comparisonMetrics) {
      ...
      const actualValue = actual[metric as keyof ActualPerformance] as
        | number
        | null;
      ...
      if (simulated && actualValue !== null) {
        ...
        hitRates.p50[metric] =
          actualValue >= simulated.p25 && actualValue <= simulated.p75;
        hitRates.p75[metric] =
          actualValue >= simulated.p5 && actualValue <= simulated.p95;
        hitRates.p95[metric] =
          actualValue >= simulated.min && actualValue <= simulated.max;
    ...
    if (metricCount === 0) {
      ...
      const avgError =
        Object.values(mae).reduce((a, b) => a + b, 0) /
        Object.values(mae).length;
      if (avgError < 0.05) {
  ...
  private async runScenarioComparisons(
    ...
    for (const scenarioName of scenarios) {
      ...
      const adjustedPerformance = this.applyMarketAdjustment(
        result,
        marketParams
      );
  ...
  private applyMarketAdjustment(
    ...
    return {
      ...
      standardDeviation:
        Math.abs(adjustedMean) * marketParams.exitMultiplierVolatility,
    };
  }
  ...
  private generateScenarioInsights(
    ...
    if (performance.p95 - performance.p5 > performance.mean * 2) {
      insights.push(
        'High outcome dispersion - significant uncertainty in returns'
      );
    }
  ...
  private generateRecommendations(
    ...
    if (validation.modelQualityScore < 70) {
      recommendations.push(
        'Consider recalibrating distribution parameters based on fund-specific data'
      );
    }
    ...
    if (irrSim && irrSim.standardDeviation > 0.15) {
      recommendations.push(
        'High IRR variance detected - consider additional scenario analysis'
      );
    }
...
describe('Backtesting API Routes', () => {
  ...
  describe('POST /api/backtesting/run', () => {
    ...
    it('rejects invalid date range', async () => {
      const response = await request(app).post('/api/backtesting/run').send({
        fundId: 1,
  ...
  describe('GET /api/backtesting/history/:fundId', () => {
    it('retrieves backtest history for a fund', async () => {
      const response = await request(app).get('/api/backtesting/history/1');
    ...
    it('supports pagination', async () => {
      const response = await request(app).get(
        '/api/backtesting/history/1?limit=5&offset=0'
      );
  ...
  describe('GET /api/backtesting/scenarios', () => {
    it('returns available historical scenarios', async () => {
      const response = await request(app).get('/api/backtesting/scenarios');
  ...
  describe('GET /api/backtesting/health', () => {
    it('returns health status', async () => {
      const response = await request(app).get('/api/backtesting/health');
...
router.post(
  ...
  async (req: Request, res: Response) => {
    const correlationId =
      (req.headers['x-correlation-id'] as string) || `bt_${Date.now()}`;

    try {
      console.log(
        `[BACKTESTING] Starting backtest ${correlationId} for fund ${req.body.fundId}`
      );

      const result = await backtestingService.runBacktest(req.body);

      console.log(
        `[BACKTESTING] Completed backtest ${correlationId} in ${result.executionTimeMs}ms`
      );
...
router.get('/history/:fundId', async (req: Request, res: Response) => {
  try {
    ...
    const history = await backtestingService.getBacktestHistory(fundId, {
      limit,
      offset,
    });
...
router.post(
  '/scenarios/compare',
  validateRequest(CompareScenarioSchema),
  async (req: Request, res: Response) => {
    const correlationId =
      (req.headers['x-correlation-id'] as string) || `sc_${Date.now()}`;

    try {
      const { fundId, scenarios, simulationRuns } = req.body;

      console.log(
        `[BACKTESTING] Comparing ${scenarios.length} scenarios for fund ${fundId}`
      );
      ...
      console.error(
        `[BACKTESTING] Scenario comparison ${correlationId} failed:`,
        error
      );
...
describe('Backtesting Integration', () => {
  describe('Full Backtest Workflow', () => {
    it('runs a complete backtest workflow', async () => {
      // 1. Check health
      const healthResponse = await request(app).get('/api/backtesting/health');
      expect(healthResponse.status).toBe(200);
      expect(healthResponse.body.status).toBe('healthy');

      // 2. Get available scenarios
      const scenariosResponse = await request(app).get(
      ...
      expect(
        backtestResponse.body.validationMetrics.modelQualityScore
      ).toBeGreaterThanOrEqual(0);

      // 4. Check history
      const historyResponse = await request(app).get(
        '/api/backtesting/history/1'
      );
...
The Backtesting API enables validation of Monte Carlo simulation accuracy
against historical fund performance data, and provides scenario replay analysis
for stress testing.
...
1. **Simulation Validation** - Compare Monte Carlo predictions to actual fund
   performance
