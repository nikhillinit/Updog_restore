...
**Issue**: 517 React hook errors in client tests **Root Cause**: jsdom/RTL
environment configuration (NOT dependency conflict) **Proof**: React
deduplicated to 18.3.1, server tests work, hook errors persist **Impact**: 34
client test files failing, 1478 tests passing but unusable
...
**Total Time**: ~25 minutes **Success Rate**: High (Codex handles context +
validation)
...
**Total Time**: ~30 minutes **Success Rate**: Medium (requires manual diagnosis)

### Path C: Agent-Heavy (If Stuck - 45 min)

Use error-debugging agent if Codex doesn't resolve in 2 attempts.

See [WEEK2.5-PHASE2-AGENT-STRATEGY.md](WEEK2.5-PHASE2-AGENT-STRATEGY.md) for
details.
...
**File**: `tests/setup/jsdom-setup.ts` **Problem**: Missing RTL cleanup between
tests
...
1. Review [WEEK2.5-PHASE2-AGENT-STRATEGY.md](WEEK2.5-PHASE2-AGENT-STRATEGY.md)
...
**Status**: Ready for Phase 2 execution **Priority**: HIGH (blocking client test
development) **Estimated Time**: 25-50 minutes **Recommended Start**:
Codex-first workflow (Path A)
