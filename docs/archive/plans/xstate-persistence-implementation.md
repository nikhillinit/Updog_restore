...
**Date:** 2025-12-01 **Status:** Ready for Implementation **Related:** ADR-016
in DECISIONS.md **Architect:** Multi-AI Consensus (Gemini + OpenAI + Claude)

## Executive Summary

Implement invoke-based persistence pattern for modeling wizard state machine to
fix critical data loss issue where navigation executes before persistence
completes.

**Problem:** Lines 806-820 execute `['goToNextStep', 'persistToStorage']` in
wrong order **Solution:** Dedicated `persisting` state with retry logic and
error recovery **Approach:** 3 incremental PRs with 15 TDD test cases
...
**Goal:** Add persistence tracking infrastructure without breaking existing
functionality
...
- [x] 4 service tests pass (success, quota error, security error, context
      initialization)
...
describe('PR #2: States & Retry Logic', () => {
  ...
  it('should execute navigation after successful persistence', async () => {
    ...
    await waitFor(actor, (state) =>
      state.matches({ active: { editing: 'idle' } })
    );
  ...
  it('should transition to persistFailed after 3 failed retries', async () => {
    ...
    await waitFor(actor, (state) =>
      state.matches({ active: { editing: 'persistFailed' } })
    );
...
- [x] 6 new actions: setNavigationIntent, executeNavigationIntent,
      increment/resetRetryCount, set/clearPersistenceError
...
**Goal:** Remove synchronous `persistToStorage` action calls from navigation
transitions
...
**Rationale:** Auto-save timer will persist within 30s. No need for immediate
persistence on config change.

#### 3. Verify No Direct persistToStorage Calls

Search codebase for any remaining direct calls to `persistToStorage()` function
and ensure they're removed or refactored to use the state machine pattern.
...
describe('PR #3: Old Pattern Removal', () => {
  ...
  it('COMPREHENSIVE: should complete full wizard flow with persistence', async () => {
    ...
    await waitFor(
      actor,
      (state) => state.context.currentStep === 'sectorProfiles'
    );
    expect(localStorage.getItem('modeling-wizard-progress')).toBeTruthy();
    ...
    actor.send({
      type: 'SAVE_STEP',
      step: 'sectorProfiles',
      data: validSectorProfilesData,
    });
    actor.send({ type: 'NEXT' });
    await waitFor(
      actor,
      (state) => state.context.currentStep === 'capitalAllocation'
    );
    ...
    const stored = JSON.parse(
      localStorage.getItem('modeling-wizard-progress')!
    );
...
**Atomic PR strategy ensures** clean rollback without breaking dependent
changes.
...
**Implementation Ready:** This plan is comprehensive, tested, and ready for
execution in a fresh session with full context budget.
