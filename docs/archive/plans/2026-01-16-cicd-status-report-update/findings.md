...
> **Session:** 2026-01-16 **Last Updated:** 2026-01-16T08:35:00Z
...
**Learning:** Jobs that only run on `push` to `main` won't be validated by PR
CI.

**Application to Current Task:**

- The `report-metrics` job issue documented in the 2026-01-15 session is a
  perfect example
...
- When updating the CI/CD Status Report format, verify no downstream tools
...
**Learning:** Filesystem walking differs across Windows/Linux; use
`git ls-files` for determinism.
...
- **Evidence:** docs/plans/2026-01-15-ci-routing-bundle-fix/progress.md lines
  169-172
...
- **Evidence:** docs/plans/2026-01-15-ci-routing-bundle-fix/progress.md lines
  153-168
- **Key Change:** Testcontainers tests moved to dedicated workflow
  (label-triggered)
...
The "Recently Fixed Issues" section must expand from 3 issues (all PR #409) to
include impacts from all 7 PRs.
...
**Original Report:** 15 active workflows **Actual Count:** 17 active workflows

**Missing from Original Report:**

1. `docs-routing-check.yml` - Discovery routing validation (this is what PR #409
   fixed!)
...
**Original Report:** 674 instances **Current Count:** 866 instances (128
increase)
...
**Resolution:** Use CI-PHILOSOPHY.md as authoritative source, add clarifying
note in report.
...
The project uses **baseline comparison, not absolute perfection** as merge
criteria:
...
**Problem:** Integration tests using testcontainers tried to run in GitHub
Actions service containers (Docker unavailable).
...
**Configuration:** These tests removed from `vitest.config.int.ts` to prevent CI
failures.

### Impact on Report

Add new section documenting testcontainers architecture under "Test
...
**Original Report:** Tests run on Node 18.x and 20.x **Current State:** Node
18.x removed, requires >=20.19.0
...
**Where am I going?** Phase 2 - Information gathering and verification of all
numbers.

**What is the goal?** Updated CI/CD Status Report reflecting current state as of
2026-01-16.
...
**Impact on Report:** Note Vercel as "External integration or not applicable"
rather than "FAIL".
...
**Recommendation:** Mark as "IMPROVED (infrastructure fixes in PR #418)" rather
than blanket "FAIL".
...
- 6 modules: XIRR (50), Waterfall Tier (15), Waterfall Ledger (14), Fees (10),
  Capital Alloc (20), Exit Recycling (20)
