...
**Created:** 2025-12-19 **Status:** READY FOR EXECUTION **Branch:**
`week2-foundation-hardening` **PR:** #293 (OPEN - awaiting review)
**Distinguishing Label:** Week 2.5 (Test Infrastructure Deep Dive with Full Tool
Inventory)
...
**CRITICAL CORRECTION:** Initial assessment of "116 test files fail to load" was
INCORRECT.
...
- **Collection Phase:** [Done] ALL test files load successfully
- **Execution Phase:** [Blocked] Tests run but assertions fail

**Key Finding:** Tests ARE loading and executing - they're failing assertions,
NOT failing to load.
...
**Commits:** 26 total (20 cherry-picked + 6 new fixes) **TypeScript Baseline:**
387 errors (0 new errors) **Build Status:** [Done] PASSING (Vite + server)
**Test Status:** [Warning] **46 test files failing** (394 test failures out of
2122 tests)
...
- **node-setup.ts Module Hooks** (217a11d9): Removed module-level
  beforeAll/afterAll

#### Critical Discovery: Module-Level Hook Anti-Pattern

**Root Cause:** Vitest hooks executing during module import, before test runner
initialized **Error Pattern:**
...
**Root Cause:** API tests in `tests/api/*.test.ts` import the REAL PostgreSQL
pool from `server/db/pg-circuit.ts` instead of using the mocked database.
Database mock infrastructure exists but tests bypass it.
...
- **Option A (Quick):** Add `vi.mock('../../server/db/pg-circuit')` to each API
  test
...
**Root Cause:** Tests use `vi.mock('../../server/db')` but mock only exports
`{ databaseMock, poolMock }`. Tests expect individual functions.

**Example:** `tests/integration/circuit-breaker-db.test.ts`

**Fix Strategy:** Update `tests/helpers/database-mock.cjs`:
...
**Root Cause:** Schema validation assertions expect
`safeParse().success === true` but schemas reject test data. Likely:
...
**Status:** Tests execute but fail for various reasons **Approach:** Individual
...
**CRITICAL DISCOVERY:** Marketplace plugins are DISABLED (0 active agents
despite CAPABILITIES.md claiming "200+")
...
- **perf-guard** - Bundle analysis, build time metrics (Extended Thinking:
  $0.06)
...
- **code-simplifier** - Simplify code while preserving functionality (model:
  opus)
...
Task({
  ...
  prompt: 'Review for CLAUDE.md adherence',
});
...
- **verification-before-completion** - MANDATORY (auto-activates before claiming
  done)
...
- **/phoenix-truth** `[focus=xirr|waterfall|fees|capital|recycling|all]` - Run
  119 truth cases
- **/phoenix-phase2** `[goal] seed=<int> iters=<int>` - Phase 2 probabilistic
  validation
...
mcp__multi -
  ...
  collab__ai_consensus({
    question:
      'Should we use Option A (vi.mock), B (global mock), or C (test database)?',
...
2. **Phase 2: Pattern Analysis**
   ...
   Task({
     subagent_type: 'schema-drift-checker',
     prompt:
       'Diagnose schema alignment across Migration -> Drizzle -> Zod -> Mock layers. Focus on server/db exports vs database-mock.cjs exports. List all missing mock contracts.',
   });
...
- `CAPABILITIES.md` - Capability inventory (**NOTE:** Claims "200+ marketplace
  agents" but 0 active)
...
**CRITICAL NOTE:** CAPABILITIES.md claims "200+ marketplace agents" but ALL
marketplace plugins are DISABLED. All functionality comes from project-level and
user-level plugins.
...
**Last Updated:** 2025-12-19 **Author:** Claude Code (Sonnet 4.5) **PR:** #293
**Test Status:** 46/116 files failing (57.8% pass rate) -> Target: 90%+ pass
rate **Tool Suite:** 250+ components (Phoenix + Project + Skills + Commands +
MCP) **Distinguishing Label:** Week 2.5 (Complete tool inventory + execution
plans)
