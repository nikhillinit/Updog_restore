...
**Goal:** Register orphaned backend routes, obtain scenario-comparison approval,
improve frontend test coverage, and clean up duplicate code.

**Architecture:** Incremental registration with feature flags, test
infrastructure fixes before test authoring, and safe deletion of verified dead
code.

**Tech Stack:** Express.js, TypeScript, Vitest, React Testing Library, Redis,
PostgreSQL
...
<research_thinking> <initial_analysis> Task: Execute 5 prioritized improvements
to backend and testing infrastructure
...
**Priority:** CRITICAL - Blocks all test-related work **Effort:** 15-30 minutes
**Risk:** LOW
...
**Priority:** HIGH **Effort:** 1-2 hours (down from 2-3 days) **Risk:** LOW
(routes are complete, just unregistered)
...
export const features = {
  // ... existing flags ...
  portfolioIntelligence: flag(
    process.env['ENABLE_PORTFOLIO_INTELLIGENCE'],
    false
  ),
};
```
...
**Location:** `server/routes.ts` (near line 770, after other route
registrations)
...
if (features.portfolioIntelligence) {
  const portfolioIntelligenceRoutes =
    await import('./routes/portfolio-intelligence.js');
...
**Expected:** 200 OK with strategies array (may be empty) OR 401 if auth
required
...
**Priority:** MEDIUM **Effort:** 1 hour documentation + EXTERNAL approval
...
**Status:** PENDING APPROVAL **Date:** 2025-12-28 **Deciders:** [Product Owner],
[Engineering Lead]
...
**Priority:** MEDIUM **Effort:** 4-6 weeks (realistic estimate) **Risk:** MEDIUM
...
export const mockInvestmentFactory = (
...
**Priority:** LOW **Effort:** 1-2 hours **Risk:** LOW (operational routes, not
user-facing)
...
**Priority:** LOW **Effort:** 30 minutes **Risk:** LOW (verified dead code)
