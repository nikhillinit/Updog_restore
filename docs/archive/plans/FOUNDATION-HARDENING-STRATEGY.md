...
**Branch:** `claude/plan-foundation-hardening-TZ75k` **Created:** 2025-12-24
**Status:** PLANNING
...
**Files affected:** 2 test files, ~20+ test failures **Root cause:**
`StreamingMonteCarloEngine` connection pool initialization fails when
...
**Files affected:** 1 test file, ~35 test failures **Root cause:** Mixed - rate
limiting assertions, security test expectations
...
**Count:** 22 test files skipped, 415 individual tests skipped **Root cause:**
TBD - need investigation
...
The security tests have **INVERTED EXPECTATIONS** - they expect 201 when
validation correctly returns 400.
...
const requests = Array(150)
  .fill(null)
  .map(() => request(app).get('/api/portfolio/strategies'));
```
...
- [FOUNDATION-HARDENING-PLAN.md](../../FOUNDATION-HARDENING-PLAN.md) - Original
  plan
...
**Author:** Claude (ultrathink planning mode) **Last Updated:** 2025-12-24
