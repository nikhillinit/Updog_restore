...
After PR #444 introduced `find_repo_root()` for CWD-independence, a regression
occurred where absolute local paths (e.g.,
`C:/dev/Updog_restore/docs/skills/...`) were baked into `SKILLS_INDEX.md`.
...
**Rationale:** SKILLS_INDEX.md is in the same directory as the REFL files, so
filename-only links work correctly on GitHub.
...
- Links as filename-only:
  `(REFL-001-dynamic-imports-prevent-test-side-effects.md)`
