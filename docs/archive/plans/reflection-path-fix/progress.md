...
- [x] Hardening A: Path separator normalization (integrated into
      normalize_rel_path)
...
**Problem Identified:** User reported SKILLS_INDEX.md contained absolute paths
after PR #444 merge.

**Analysis:**

- `meta['path'] = str(f).replace('\\', '/')` was converting absolute Path
  objects to strings
