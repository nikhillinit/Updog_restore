...
**Review Date:** 2025-12-29 **Original Plan:**
`docs/plans/2025-12-29-phoenix-phases-1.4-1.7-completion.md` **Status:**
...
Critical review of the implementation plan reveals **7 major oversights** that
would cause plan failure if not addressed. The plan incorrectly assumes
infrastructure must be built from scratch when it already exists, uses wrong
file paths, and underestimates semantic complexity in the Capital Allocation
module.
...
**Plan assumes:** Create new adapters from scratch **Reality:** Fully
implemented adapters already exist
...
**Impact:** Tasks 1.4.1 and 1.5.1 can be reduced to "Review existing adapter"
(30 min vs 2+ hours).
...
**Reality:** Comprehensive test file exists at
`tests/unit/truth-cases/capital-allocation.test.ts` (422 lines)
...
**Impact:** Task 1.4.3 becomes "Run existing tests and capture baseline" not
...
**Plan uses:** `tests/truth-cases/adapters/` **Correct path:**
...
**Impact:** Task 1.5.3 requires creating a new test file from scratch (not just
running existing).
...
**Plan assumes:** 100% TRUTH_CASE_ERROR pattern like XIRR/Fees **Reality:**
Capital Allocation has fundamental semantic discrepancies
...
const ALLOCATION_OVERRIDES: Record<
  string,
  Array<{ cohort: string; amount: number }>
> = {
...
**Impact:** May require Phase 1B (bug fix) not Phase 1A (cleanup) for some
scenarios.
...
**Plan assumes:** Simple adapter + production function pattern **Reality:** Two
distinct execution engines exist
...
**Plan assumes:** `calculateCapitalAllocation(input)` **Reality:** Three
distinct functions:
...
**Probability:** HIGH (evidence in code) **Impact:** MEDIUM-HIGH
...
14 of 20 CA cases require period-loop engine, which may have different
validation patterns.
...
1. **Update all file paths** from `tests/truth-cases/` to
   `tests/unit/truth-cases/`
...
1. **Create CA-SEMANTIC-MODEL-ADR.md** documenting capacity vs cash model
   decision
...
With these refinements, Phase 1.4-1.7 remains achievable but requires more
nuanced handling of Capital Allocation semantic discrepancies.

**Revised Confidence:** MEDIUM-HIGH (was HIGH) **Primary Risk:** Semantic model
divergence requiring Phase 1B work
