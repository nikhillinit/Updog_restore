...
**Architecture:** 5-tier prioritization (CI blockers -> critical path -> quick
wins -> technical debt -> permanent quarantine). Each tier processes tests in
parallel where possible, with verification gates between tiers.
...
const redis =
  process.env.NODE_ENV === 'test' || process.env.DEMO_CI
    ? new RedisMock()
...
The test at line 130 expects Newton-Raphson to handle 10x returns in 6 months.
...
Tests in this directory are intentionally skipped due to external dependencies
or blocked features. Each must have documented exit criteria.
