...
**Version:** 2.1 (Incorporating Human Expert Review)
 ...
 */
export function saveToStorage<T>(key: string, data: T, schema?: z.ZodType<T>): void {
  if (!ALLOWED_KEYS.has(key)) {
...
export const SectorProfilesSchema = z.object({
  ...
  (data) => {
    const total = data.sectorProfiles.reduce((sum, s) => sum + s.allocation, 0);
...
import { GeneralInfoSchema, CapitalAllocationSchema, FeesExpensesSchema } from '@/schemas/wizard';
...
async function calculatePacing(ctx: ModelingWizardContext): Promise<PacingResult> {
  ...
  const quarters = (data.investmentPeriod || CALCULATION_DEFAULTS.DEFAULT_INVESTMENT_PERIOD_MONTHS) / 3;
...
async function calculateSector(ctx: ModelingWizardContext): Promise<CohortBenchmarks> {
  ...
  return {
    ...
    gradToA: analysis.graduationRates?.seriesA || CALCULATION_DEFAULTS.DEFAULT_GRADUATION_TO_A
  };
}
...
function generateSyntheticPortfolio(input: {
  ...
  for (let i = 0; i < numberOfCompanies; i++) {
    ...
    const months = SYNTHETIC_MONTHS_TO_GRAD_MIN + Math.floor(Math.random() * monthsRange);
