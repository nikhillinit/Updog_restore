...
export class BoundedMemoryCache implements Cache {
  ...
  constructor(options: { maxSize?: number; defaultTTL?: number; cleanupInterval?: number } = {}) {
    ...
    logger.info(
      { maxSize: this.maxSize, defaultTTLSeconds: this.defaultTTL },
      '[cache] Bounded memory cache initialized'
    );
  }
  ...
  private cleanup(): void {
    ...
    if (cleaned > 0) {
      this.updateSize();
      logger.debug(
        { cleaned, remaining: this.data.size, maxSize: this.maxSize },
      ...
      logger.info(
        {
          ...
          hitRatePercent: Number((this.getHitRate() * 100).toFixed(1)),
