...
export function memoryStore(): IdempotencyStore {
  ...
  return {
    async get<T>(key: string) {
      return m['get'](key) as IdemRecord<T> | undefined;
    },
    async set<T>(key: string, rec: IdemRecord<T>) {
      m['set'](key, rec);
    },
 ...
 */
export function withIdempotency(options?: { ttlSeconds?: number; store?: IdempotencyStore }) {
