...
router['post']('/', requireRole('reserve_admin'), async (req: Request, res: Response) => {
  try {
    ...
    const partners = await db
      ...
      .where(isNull(approvalPartners.deactivated));
    ...
    const approvals = await db
      .select({
        ...
        signatureCount: db.$count(
          approvalSignatures,
          eq(approvalSignatures.approvalId, reserveApprovals.id)
        ),
      })
      .from(reserveApprovals)
      .where(and(eq(reserveApprovals.status, status), gte(reserveApprovals.expiresAt, new Date())))
...
async function canPartnerSign(email: string, approvalId: string): Promise<boolean> {
  ...
  const [existingSignature] = await db
    ...
    .from(approvalSignatures)
    .where(
      and(eq(approvalSignatures.approvalId, approvalId), eq(approvalSignatures.partnerEmail, email))
    );
...
async function notifyPartners(
  partners: ApprovalPartner[],
  approval: ReserveApproval
): Promise<void> {
  const config = getConfig();
  const baseUrl =
    ((config as Record<string, unknown>)['publicUrl'] as string) ??
