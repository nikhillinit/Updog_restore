...
const CohortFundAccessSchema = z.object({
  fundId: z.coerce.number().int().positive(),
});

function requireCohortFundAccessFrom(
  source: 'body' | 'query'
): (req: Request, res: Response, next: NextFunction) => void {
  return (req: Request, res: Response, next: NextFunction) => {
    const parsed = CohortFundAccessSchema.safeParse(req[source]);
    if (!parsed.success) {
      return res.status(400).json({
        error: 'validation_error',
        message: source === 'body' ? 'Invalid request fundId' : 'Invalid query parameter fundId',
...
const requireCohortBodyFundAccess = requireCohortFundAccessFrom('body');
const requireCohortQueryFundAccess = requireCohortFundAccessFrom('query');
    ...
    try {
      ...
      const [existingTaxonomy] = await db
        ...
        .where(
          and(
            eq(sectorTaxonomy.fundId, fundId),
            eq(sectorTaxonomy.taxonomyVersion, taxonomyVersion)
          )
        )
        .limit(1);
