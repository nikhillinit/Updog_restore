...
export class TypedCircuitBreaker<T = unknown> implements BreakerLike {
  ...
  constructor(options: CircuitBreakerOptions) {
    ...
    this.breaker = new CircuitBreaker<T>(
      options,
      async () => {
        throw new Error('Use run method');
      },
      async () => {
        throw new Error('Use run method');
      }
  ...
  async run<R>(operation: () => Promise<R>, fallback?: () => Promise<R>): Promise<R> {
