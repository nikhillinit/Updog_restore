...
function generateExecutiveSummary(input: CommentaryInput): CommentarySection {
  ...
  return {
    ...
    content:
      `${metrics.fundName} delivered ${performanceDesc} in ${metrics.quarter} ${metrics.year}, ` +
...
function generatePerformanceOverview(input: CommentaryInput): CommentarySection {
  ...
  return {
    ...
    content:
      `Fund performance metrics reflect the current portfolio valuation as of ${metrics.quarter} ${metrics.year}. ` +
...
function generatePortfolioActivity(input: CommentaryInput): CommentarySection {
  ...
  if (metrics.followOnInvestments) {
    activityBullets.push(
      `${metrics.followOnInvestments} follow-on investments to support portfolio growth`
    );
  }
  if (metrics.exited > 0) {
  ...
  return {
    ...
    content:
      `This quarter saw continued active portfolio management across the fund. ` +
    ...
    bullets:
      activityBullets.length > 0
        ? activityBullets
        : ['Normal portfolio operations during the quarter'],
  };
}
...
function generateLookingAhead(input: CommentaryInput): CommentarySection {
  ...
  return {
    ...
    title: 'Looking Ahead',
    content:
      `As we move into the next quarter, ${metrics.fundName} remains focused on maximizing value for our limited partners. ` +
...
export async function generateCommentary(input: CommentaryInput): Promise<CommentaryOutput> {
  ...
  return {
    ...
    keyMetrics: [
      {
        label: 'NAV',
        value: formatCurrency(input.metrics.nav),
        trend: getTrend(input.metrics.navChange ?? 0),
      },
