...
export const agentConfigs = {
  syntax: {
    specialty: 'syntax',
    maxConcurrency: 3,
    patterns: [/SyntaxError/, /Unexpected token/, /Missing semicolon/, /Unterminated/],
  },
  assertion: {
    specialty: 'assertion',
    maxConcurrency: 2,
    patterns: [/Expected .* to be/, /toBe|toEqual|toMatch/, /AssertionError/, /received.*expected/],
  },
  integration: {
    specialty: 'integration',
    maxConcurrency: 1,
    patterns: [/database|Database/, /redis|Redis/, /connection|Connection/, /timeout.*query/i],
  },
  e2e: {
    specialty: 'e2e',
    maxConcurrency: 2,
  ...
  classify(failures: TestFailure[]): TestFailure[] {
    return failures.map((failure) => {
      ...
      for (const [agentType, config] of Object.entries(agentConfigs)) {
        const matchCount = config.patterns.filter(
          (pattern) => pattern.test(failure.error) || pattern.test(failure.stack)
        ).length;
...
export class RepairRouter {
  ...
  async route(failures: TestFailure[]): Promise<RepairPlan> {
    ...
    return {
      syntax: classified.filter((f) => f.type === 'syntax' && (f.confidence ?? 0) > 0.7),
      assertion: classified.filter((f) => f.type === 'assertion' && (f.confidence ?? 0) > 0.6),
      integration: classified.filter(
        (f) =>
          f.type === 'integration' || (/database|redis/i.test(f.stack) && (f.confidence ?? 0) > 0.5)
      ),
      e2e: classified.filter((f) => f.type === 'e2e' || /\.e2e\./.test(f.file)),
      manual: classified.filter((f) => f.type === 'unknown' || (f.confidence ?? 0) < 0.5),
    };
  }
}
  ...
  async executeRepairs(failures: TestFailure[]): Promise<{
    ...
    for (const [type, agentFailures] of Object.entries(plan)) {
      ...
      if (agent) {
        repairPromises.push(
          agent
            .execute({ failures: agentFailures })
