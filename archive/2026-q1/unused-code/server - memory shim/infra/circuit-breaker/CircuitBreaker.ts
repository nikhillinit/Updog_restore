...
export class CircuitBreaker<T = unknown> {
  ...
  onStateChange(listener: (event: StateChangeEvent) => void) {
  ...
  private maybeAdaptThreshold() {
    ...
    if (recentSuccessRate > 0.95) {
      this.effectiveFailureThreshold = Math.min(
        cfg.max,
        Math.round(this.effectiveFailureThreshold * (1 + cfg.rate))
      );
