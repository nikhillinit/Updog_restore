...
import type { AgentExecutionContext } from './src/index';
import { BaseAgent, getThread, buildConversationHistory, clearAllThreads } from './src/index';
...
async function runMultiAgentWorkflow() {
  ...
  const analyzerResult = await analyzer.execute({ files: ['test.ts'] }, 'analyze', {
    files: ['test.ts'],
  });
  ...
  const fixerResult = await fixer.execute({ issues: analyzerResult.data?.issues || [] }, 'fix', {
  ...
  console.log('✓ Step 3: Validator verifies fixes (with analyzer + fixer context)\n');

  const validatorResult = await validator.execute({ files: ['test.ts'] }, 'validate', {
  ...
  if (thread) {
    ...
    thread.turns.forEach((turn, idx) => {
      ...
      console.log(`    Content: ${turn.content.substring(0, 100)}...`);
