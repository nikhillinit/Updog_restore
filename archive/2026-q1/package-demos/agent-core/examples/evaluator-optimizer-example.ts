...
Detailed feedback on what needs improvement
  ...
  const generator: GeneratorFunction<string> = createGenerator<string>(
    generatorPrompt,
    mockLlmCall
  );
  const evaluator: EvaluatorFunction<string> = createEvaluator<string>(
    evaluatorPrompt,
    mockLlmCall
  );
...
async function typedContentExample(): Promise<void> {
  ...
  const generator: GeneratorFunction<CodeArtifact> = async (_task, _context?) => {
    return {
  ...
  const evaluator: EvaluatorFunction<CodeArtifact> = async (content, _task) => {
