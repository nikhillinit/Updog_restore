...
import type { BacktestCase } from '../src/BacktestReporter';
...
function generateSampleBacktestCases(): BacktestCase[] {
  ...
  for (let i = 16; i <= 25; i++) {
    ...
    const patterns: Array<{
      pattern: 'evaluator-optimizer' | 'router' | 'orchestrator' | 'prompt-cache';
      taskType: 'typescript-error' | 'react-component' | 'test-failure' | 'performance';
    }> = [
...
async function runBacktestExample() {
  ...
  console.log(
    `  • Break-Even: ${report.summary.breakEvenCases} cases (${report.summary.totalCases >= report.summary.breakEvenCases ? 'ACHIEVED' : 'pending'})`
  );
  console.log('');
  ...
  for (const [pattern, metrics] of Object.entries(report.patternAnalysis)) {
    ...
    console.log(
      `    - ${metrics.avgSpeedup.toFixed(1)}x speedup, $${metrics.totalCostSavings.toFixed(2)} saved`
    );
  ...
  console.log(
    `  ✓ Success Rate by Pattern (${report.charts.successRateByPattern.data.length} bars)`
  );
  console.log(
    `  ✓ Cost Savings Over Time (${report.charts.costSavingsOverTime.series[0].data.length} data points)`
  );
  console.log(`  ✓ Quality Distribution (${report.charts.qualityDistribution.bins.length} bins)`);
  console.log(
    `  ✓ Router Accuracy (${report.charts.routerAccuracyByTaskType.data.length} segments)`
  );
