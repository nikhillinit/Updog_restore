...
**Duration**: 5 days
**Goal**: Establish event-sourcing foundation and fix CI stability
**Stack**: Express + BullMQ + Drizzle (existing architecture)
...
export const fundSnapshots = pgTable('fund_snapshots', {
  ...
  type: varchar('type', { length: 20 }).$type<'manual' | 'scheduled' | 'auto'>().notNull(),
  description: text(),
...
export class FundProjector {
  ...
  private applyValuationUpdate(state: FundState, payload: any): FundState {
    return {
      ...state,
      totalValue: state.totalValue - this.getCompanyValue(state, payload.companyId) + payload.newValuation,
      portfolioCompanies: state.portfolioCompanies.map(company =>
        company.id === payload.companyId
  ...
  private applyReserveAllocation(state: FundState, payload: any): FundState {
    const existingReserve = state.reserves.find(r => r.companyId === payload.companyId);
  ...
  private getCompanyValue(state: FundState, companyId: string): number {
    return state.portfolioCompanies.find(c => c.id === companyId)?.currentValuation || 0;
  }
...
describe('FundProjector', () => {
  ...
  it('should apply INVESTMENT_MADE event', () => {
    ...
    initialState.portfolioCompanies = [
      { id: 'company-a', name: 'Company A', totalInvestment: 0, currentValuation: 0, ownership: 0 }
    ];
...
const getTimelineSchema = z.object({
  ...
  query: z.object({
    ...
    limit: z.string().transform(Number).pipe(z.number().int().positive()).default('100')
  })
});
...
export default function() {
  ...
  const stateRes = http.get(
    `${BASE_URL}/api/funds/${FUND_ID}/state`,
...
|--------|--------|--------|
| Fund state query p95 | <200ms | ___ |
| Timeline query p95 | <200ms | ___ |
| Snapshot creation p95 | <500ms | ___ |
| Test pass rate | >95% | ___ |
| Flaky test count | <5 | ___ |
| PR review time | <4 hours | ___ |
