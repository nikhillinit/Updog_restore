...
| 2   | `server/routes/liquidity.ts` POST /forecast passed `transactions                                                                                         |         | []`unfiltered into`generateLiquidityForecast` (executed and cancelled rows as upcoming) | LOW          | Low       | Low          | Good       | 1 file                                              | Trivial | Fixed (upcoming-status filter) |
