...
function parse<T>(schema: z.ZodType<T, z.ZodTypeDef, unknown>, value: unknown, path: string): T {
  ...
  throw new AdmissionRefusal(
    result.error.issues.map((issue) => ({
      code: issue.message.startsWith('TIME_ORIGIN_UNRESOLVED')
        ? 'TIME_ORIGIN_UNRESOLVED'
        : 'INVALID_INPUT',
      path: issuePath(issue.path) || path,
      message: issue.message,
      support: issue.message.startsWith('TIME_ORIGIN_UNRESOLVED') ? 'incomplete' : 'invalid',
    }))
  );
}
...
export function fingerprintCapitalSource(source: CapitalRawSource): {
  ...
  const rawConfigHash = sha256CanonicalJson(source.config.raw);
...
function normalizeSource(
  ...
  function unit(path: string, allowed: readonly string[]): CapitalUnitDeclarationsV1[string] {
    ...
    if (value === undefined || !allowed.includes(value))
      refuse(
        ...
        path,
        'An exact-path source-unit declaration is required',
        'incomplete'
      );
    consumed.add(path);
    return value;
  ...
  } else if (raw.feeProfiles?.length) {
...
function selectConstruction(
  ...
  const selectedIds = new Set(
    [...selectedAllocations].map((i) => raw.capitalPlanAllocations![i]!.id)
  );
  return {
    allocations: (raw.allocations ?? []).flatMap((item, i) =>
  ...
  return inputs.map((input) => {
    ...
    throw new AdmissionRefusal(
      parsed.error.issues.map((issue): CapitalIssueV1 => {
        ...
        } else if (issue.message.startsWith('Pro-rata')) {
...
export function materializeCapitalSource(args: {
  ...
  try {
    if (
      args.expectedInterpretationVersion !== undefined &&
      args.expectedInterpretationVersion !== CAPITAL_SOURCE_INTERPRETATION_VERSION
    )
      refuse(
    ...
    const fingerprint = fingerprintCapitalSource(args.source);
    const sourceBundle = parse(
      CapitalSourceBundleV1Schema,
      {
        contractVersion: 'fund-scenario-capital-source-bundle/1.0.0',
        ...fingerprint,
        publishedAt: args.source.config.publishedAt,
    ...
    // JSONB cannot carry nonfinite numbers, cycles or class instances. Keep direct-call failures typed.
        ...
        [
          {
            code: 'INVALID_INPUT',
            path: 'config',
            message: 'Source must be canonical JSON data',
            support: 'invalid',
          },
        ],
        'current_preview'
...
export function verifyPinnedCapitalSourceBundle(args: {
  ...
  try {
    ...
    if (
      sha256CanonicalJson(savedProjection) !== bundle.sourceBundleHash ||
    ...
    if (
      ...
      return failure(error.issues, 'saved_input');
    return failure(
      [
        {
          code: 'HISTORICAL_SOURCE_INTEGRITY_FAILED',
          path: 'sourceBundle',
          message: 'Saved source or its derived bundle failed integrity verification',
          support: 'invalid',
        },
      ],
      'saved_input'
