...
function reconcile(
  ...
  const roundRowCounts = new Map<string, Map<string, { total: number; beyond: number }>>();
  for (const row of rows) {
    if (row.roundId === null) continue;
    let allocation = roundRowCounts.get(row.allocationId);
    if (!allocation) {
      allocation = new Map();
      roundRowCounts.set(row.allocationId, allocation);
    }
    const counts = allocation.get(row.roundId) ?? { total: 0, beyond: 0 };
    counts.total++;
    if (row.beyondTerm) counts.beyond++;
    allocation.set(row.roundId, counts);
  }
  const exactBeyond = sum(
    models.map((model, index) => {
      ...
      return sum(
        model.rounds.map((round) => {
          const outside =
            roundRowCounts.get(model.input.allocationId)?.get(round.input.roundId)?.beyond ?? 0;
  ...
  const allBeyond =
    ...
    models.every(
      (model, index) =>
        !amounts[index] ||
        model.rounds.every((round) => {
          const counts = roundRowCounts.get(model.input.allocationId)?.get(round.input.roundId);
          return round.cost.isZero() || (counts?.total ?? 0) === (counts?.beyond ?? 0);
        })
    );
  return {
    reconciliation: {
...
export function calculateCapitalPlanningV1(args: {
  ...
  /** Server-owned copies; client wire admission must not accept these. */
  ...
  let resolvedBenchmarks: ReturnType<typeof resolveCapitalPlanningDraftV1> | undefined;
  if (benchmarkSnapshots && benchmarkSnapshots.length > 0) {
    const draftInput = structuredClone(input);
    for (const snapshot of benchmarkSnapshots) {
      const allocation = draftInput.allocations.find(
      ...
      if (target.kind === 'entry') delete allocation.entryFinancing;
        ...
        const round = allocation.followOnRounds.find((item) => item.roundId === target.roundId);
        if (round) delete round.financing;
      }
    }
    resolvedBenchmarks = resolveCapitalPlanningDraftV1({
      draft: {
        input: draftInput,
        benchmarkSelections: benchmarkSnapshots.map((snapshot) => ({
          target: snapshot.target,
          selector: snapshot.selector,
          ...(Object.prototype.hasOwnProperty.call(snapshot, 'overrides')
    ...
    if (JSON.stringify(resolvedBenchmarks.input) !== JSON.stringify(input))
      refuseCalculation(
        'INVALID_INPUT',
        'input',
        'Normalized financing disagrees with saved benchmark baseline, explicit intent and pinned source'
      );
  }
  ...
  const assumptions = resolvedBenchmarks
    ? applyCapitalBenchmarkProvenanceV1(
        provenance(input, source),
        resolvedBenchmarks.financingProvenance
      )
    : provenance(input, source);
  ...
  const result: CapitalPlanningResultV1 = {
    ...
    ...(resolvedBenchmarks ? { benchmarkSnapshots: resolvedBenchmarks.benchmarkSnapshots } : {}),
    construction: {
