...
export type PresentationRoundingErrorCode =
  | 'INVALID_USD_AMOUNT'
...
export class PresentationRoundingError extends Error {
  constructor(
    readonly code: PresentationRoundingErrorCode,
    message: string
  ) {
    super(message);
    this.name = 'PresentationRoundingError';
  }
}

export interface WaterfallEventEntitlements {
  totalUsd: Decimal;
  rocUsd: Decimal;
...
export interface RoundedWaterfallEvent {
  totalCents: string;
  rocCents: string;
...
export interface RoundedWaterfallRun {
  events: RoundedWaterfallEvent[];
...
export function roundWaterfallEventForPresentation(
  event: WaterfallEventEntitlements
): RoundedWaterfallEvent {
...
export function processWaterfallRunForPresentation(
  events: readonly WaterfallEventEntitlements[]
): RoundedWaterfallRun {
