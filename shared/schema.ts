...
// Import fund tables for FK references and schema definitions
...
export const forecastSnapshots = pgTable(
  ...
  (table) => ({
    ...
    sourceHashUniqueIdx: uniqueIndex('forecast_snapshots_source_hash_unique_idx')
...
export const reserveAllocations = pgTable(
  ...
  (table) => ({
    ...
    idempotencyUniqueIdx: uniqueIndex('reserve_allocations_snapshot_idem_key_idx')
      .on(table.snapshotId, table.idempotencyKey)
      .where(sql`${table.idempotencyKey} IS NOT NULL`),
    cursorIdx: index('reserve_allocations_snapshot_cursor_idx').on(
      table.snapshotId,
      table.createdAt.desc(),
      table.id.desc()
    ),
...
export const reserveStrategies = pgTable(
  'reserve_strategies',
  {
    id: serial('id').primaryKey(),
    fundId: integer('fund_id')
  ...
  (table) => ({
    fundCompanyIdx: index('idx_reserve_strategies_fund_company')['on'](
      table.fundId,
      table.companyId
    ),
  })
...
export const pacingHistory = pgTable(
  'pacing_history',
  {
    id: serial('id').primaryKey(),
    fundId: integer('fund_id')
...
export const auditLog = pgTable(
  ...
  (table) => ({
    ...
    userActionIdx: index('idx_audit_user_action')['on'](
      table.userId,
      table.action,
      table.createdAt.desc()
    ),
...
export const reserveDecisions = pgTable(
  'reserve_decisions',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    fundId: integer('fund_id')
...
export const fundStateSnapshots = pgTable(
  ...
  (table) => ({
    fundTimeIdx: index('fund_state_snapshots_fund_time_idx')['on'](
      table.fundId,
      table.snapshotTime.desc()
    ),
    ...
    bookmarkedIdx: index('fund_state_snapshots_bookmarked_idx')['on'](
      table.fundId,
      table.isBookmarked
    ),
...
export const snapshotMetadata = pgTable(
  'snapshot_metadata',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    snapshotId: uuid('snapshot_id')
      .references(() => fundStateSnapshots.id, { onDelete: 'cascade' })
      .notNull(),
...
export const restorationHistory = pgTable(
  'restoration_history',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    fundId: integer('fund_id')
    ...
    sourceSnapshotId: uuid('source_snapshot_id')
      .references(() => fundStateSnapshots.id)
      .notNull(),
  ...
  (table) => ({
    fundOperationIdx: index('restoration_history_fund_operation_idx')['on'](
      table.fundId,
      table.operationId
    ),
...
export const snapshotComparisons = pgTable(
  'snapshot_comparisons',
  {
    ...
    baseSnapshotId: uuid('base_snapshot_id')
      .references(() => fundStateSnapshots.id)
      .notNull(),
    compareSnapshotId: uuid('compare_snapshot_id')
      .references(() => fundStateSnapshots.id)
      .notNull(),
  ...
  (table) => ({
    fundComparisonIdx: index('snapshot_comparisons_fund_idx')['on'](
      table.fundId,
      table.createdAt.desc()
    ),
    snapshotsIdx: index('snapshot_comparisons_snapshots_idx')['on'](
      table.baseSnapshotId,
      table.compareSnapshotId
    ),
    uniqueComparison: unique('snapshot_comparisons_unique')['on'](
      table.baseSnapshotId,
      table.compareSnapshotId,
      table.comparisonType
    ),
    cacheIdx: index('snapshot_comparisons_cache_idx')['on'](table.cacheStatus, table.lastUsed),
    userIdx: index('snapshot_comparisons_user_idx')['on'](
      table.requestedBy,
      table.createdAt.desc()
    ),
  })
...
export const fundBaselines = pgTable(
  'fund_baselines',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    fundId: integer('fund_id')
  ...
  (table) => ({
    ...
    defaultIdx: index('fund_baselines_default_idx').on(
      table.fundId,
      table.isDefault,
      table.isActive
    ),
...
export const varianceReports = pgTable(
  'variance_reports',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    fundId: integer('fund_id')
  ...
  (table) => ({
    ...
    baselineIdx: index('variance_reports_baseline_idx')['on'](
      table.baselineId,
      table.asOfDate.desc()
    ),
...
export const performanceAlerts = pgTable(
  ...
  (table) => ({
    fundAlertIdx: index('performance_alerts_fund_idx')['on'](
      table.fundId,
      table.triggeredAt.desc()
    ),
    severityIdx: index('performance_alerts_severity_idx')['on'](
      table.severity,
      table.status,
      table.triggeredAt.desc()
    ),
    statusIdx: index('performance_alerts_status_idx')['on'](table.status, table.triggeredAt.desc()),
    metricIdx: index('performance_alerts_metric_idx')['on'](
      table.metricName,
      table.triggeredAt.desc()
    ),
    baselineIdx: index('performance_alerts_baseline_idx')['on'](
      table.baselineId,
      table.triggeredAt.desc()
    ),
...
export const fundStrategyModels = pgTable(
  'fund_strategy_models',
  {
    ...
    // Reserve strategy
    initialReservePercentage: decimal('initial_reserve_percentage', { precision: 5, scale: 4 })
      .notNull()
      .default('0.50'),
  ...
  (table) => ({
    ...
    activeIdx: index('fund_strategy_models_active_idx')['on'](
      table.isActive,
...
export const portfolioScenarios = pgTable(
  'portfolio_scenarios',
  {
    ...
    fundId: integer('fund_id')
      .notNull()
      .references(() => funds.id),
    strategyModelId: uuid('strategy_model_id')
      .notNull()
      .references(() => fundStrategyModels.id),
    ...
    dealFlowAssumption: decimal('deal_flow_assumption', { precision: 5, scale: 2 })
      .notNull()
      .default('1.00'), // Multiplier vs normal deal flow
    valuationEnvironment: decimal('valuation_environment', { precision: 5, scale: 2 })
      .notNull()
      .default('1.00'), // Valuation multiplier vs normal
    exitEnvironment: decimal('exit_environment', { precision: 5, scale: 2 })
      .notNull()
      .default('1.00'), // Exit opportunity multiplier
  ...
  (table) => ({
    ...
    strategyIdx: index('portfolio_scenarios_strategy_idx')['on'](
      table.strategyModelId,
      table.status
    ),
    ...
    sharedIdx: index('portfolio_scenarios_shared_idx')['on'](
      table.isShared,
      table.createdAt.desc()
    ),
...
export const reserveAllocationStrategies = pgTable(
  'reserve_allocation_strategies',
  {
    ...
    emergencyReservePct: decimal('emergency_reserve_pct', { precision: 5, scale: 4 }).default(
      '0.10'
    ), // Emergency reserve %

    // Deployment parameters
    maxPerCompanyPct: decimal('max_per_company_pct', { precision: 5, scale: 4 })
      .notNull()
      .default('0.20'), // Max % of reserves per company
    minDeploymentAmount: decimal('min_deployment_amount', { precision: 15, scale: 2 }).default(
      '100000'
    ),
  ...
  (table) => ({
    fundIdx: index('reserve_allocation_strategies_fund_idx')['on'](
      table.fundId,
      table.createdAt.desc()
    ),
    scenarioIdx: index('reserve_allocation_strategies_scenario_idx')['on'](
      table.scenarioId,
      table.isActive
    ),
    typeIdx: index('reserve_allocation_strategies_type_idx')['on'](
      table.strategyType,
      table.isActive
    ),
    activeIdx: index('reserve_allocation_strategies_active_idx')['on'](
      table.isActive,
      table.lastOptimizedAt.desc()
    ),
    optimizationIdx: index('reserve_allocation_strategies_optimization_idx')['on'](
      table.optimizationObjective,
      table.isActive
    ),
  })
);
...
export const performanceForecasts = pgTable(
  ...
  (table) => ({
    ...
    baselineIdx: index('performance_forecasts_baseline_idx')['on'](
      table.baselineId,
      table.createdAt.desc()
    ),
    typeIdx: index('performance_forecasts_type_idx')['on'](table.forecastType, table.status),
    methodologyIdx: index('performance_forecasts_methodology_idx')['on'](
      table.methodology,
      table.modelVersion
    ),
    horizonIdx: index('performance_forecasts_horizon_idx')['on'](
      table.forecastHorizonYears,
      table.status
    ),
  })
);
...
export const scenarioComparisons = pgTable(
  'scenario_comparisons',
  {
    ...
    baseScenarioId: uuid('base_scenario_id')
      .notNull()
      .references(() => portfolioScenarios.id),
  ...
  (table) => ({
    ...
    statusIdx: index('scenario_comparisons_status_idx')['on'](
      table.status,
      table.lastAccessed.desc()
    ),
    publicIdx: index('scenario_comparisons_public_idx')['on'](
      table.isPublic,
      table.createdAt.desc()
    ),
  })
...
export const monteCarloSimulations = pgTable(
  ...
  (table) => ({
    fundIdx: index('monte_carlo_simulations_fund_idx')['on'](
      table.fundId,
      table.simulationDate.desc()
    ),
    scenarioIdx: index('monte_carlo_simulations_scenario_idx')['on'](
      table.scenarioId,
      table.simulationDate.desc()
    ),
    forecastIdx: index('monte_carlo_simulations_forecast_idx')['on'](
      table.forecastId,
      table.simulationDate.desc()
    ),
    typeIdx: index('monte_carlo_simulations_type_idx')['on'](
      table.simulationType,
      table.simulationDate.desc()
    ),
...
export const insertReserveAllocationStrategySchema = createInsertSchema(
...
export const reallocationAudit = pgTable(
  'reallocation_audit',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    fundId: integer('fund_id')
  ...
  (table) => ({
    ...
    versionsIdx: index('idx_reallocation_audit_versions')['on'](
      table.fundId,
...
export const notionPortfolioConfigs = pgTable(
  'notion_portfolio_configs',
  {
    ...
    sharedDatabases: jsonb('shared_databases').$type<
      Array<{
        databaseId: string;
        databaseName: string;
        purpose: string;
    ...
    >(),
    automationRules: jsonb('automation_rules').$type<
      Array<{
        id: string;
        trigger: string;
        action: string;
...
export const notionDatabaseMappings = pgTable(
 ...
 */
export const comparisonConfigurations = pgTable(
  ...
  (table) => ({
    ...
    publicIdx: index('comparison_configs_public_idx')['on'](table.isPublic, table.useCount.desc()),
 ...
 * - Audit trail
 */
export const comparisonAccessHistory = pgTable(
  'comparison_access_history',
  {
    id: uuid('id').defaultRandom().primaryKey(),

    // What was accessed
    comparisonId: uuid('comparison_id').references(() => scenarioComparisons.id, {
      onDelete: 'cascade',
    }),
    configurationId: uuid('configuration_id').references(() => comparisonConfigurations.id, {
      onDelete: 'set null',
    }),
    fundId: integer('fund_id')
      .notNull()
...
export const insertComparisonConfigurationSchema = createInsertSchema(
