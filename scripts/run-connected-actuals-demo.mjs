...
import { delimiter, dirname, join, relative, isAbsolute, resolve } from 'node:path';
...
const outsideRepository = (directory) => {
  const location = relative(realpathSync(root), directory);
  return (
    location === '..' ||
...
assert.ok(outsideRepository(output), 'Connected artifacts must be outside the repository');
mkdirSync(output, { recursive: true });
assert.ok(
  outsideRepository(realpathSync(output)),
  'Connected artifacts cannot resolve inside the repository'
);
