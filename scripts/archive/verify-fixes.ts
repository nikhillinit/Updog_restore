...
const checks = [
  ...
  },
  {
    ...
    check: () => {
      try {
        ...
        return (
          serverIndex.includes('// Request ID MUST be first for correlation on all paths') &&
  ...
  },
  {
    ...
    check: () => {
      try {
        const shutdownGuard = readFileSync(
          join(process.cwd(), 'server', 'middleware', 'shutdownGuard.ts'),
          'utf-8'
        );
  ...
  },
  {
    ...
    check: () => {
      try {
        const serverIndex = readFileSync(join(process.cwd(), 'server', 'index.ts'), 'utf-8');
        return (
          serverIndex.includes('CORS_ORIGIN') &&
          serverIndex.includes('no valid origins were parsed')
        );
      } catch {
        return false;
      }
