export const ACTUALS_RESTATEMENT_PRODUCTION_PREREQUISITES = Object.freeze([
  Object.freeze({
...
export class ActualsRestatementPrerequisiteError extends Error {
  constructor() {
    super(
      `0057 production action blocked: ${ACTUALS_RESTATEMENT_PRODUCTION_PREREQUISITES.map((item) => item.reasonCode).join(', ')}`
    );
    this.name = 'ActualsRestatementPrerequisiteError';
    this.code = 'PRODUCTION_PREREQUISITES_UNAVAILABLE';
    this.prerequisites = ACTUALS_RESTATEMENT_PRODUCTION_PREREQUISITES;
  }
}
...
export function assertActualsRestatementProductionPrerequisites() {
  throw new ActualsRestatementPrerequisiteError();
}
