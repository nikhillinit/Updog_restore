...
**Domain:** REST API endpoints, validation, error handling, security **Estimated
Time:** 60 minutes **Prerequisites:** API documentation, Postman/curl,
...
This rubric covers all 48 backend API routes, focusing on request validation,
response formats, error handling, authentication/authorization, rate limiting,
and idempotency.
...
**Objective:** Verify API authentication with JWT **Steps:**
...
**Objective:** Verify Zod schema validation on all endpoints **Steps:**
...
**Objective:** Verify all API responses follow consistent structure **Steps:**
...
**Objective:** Verify cursor-based pagination (not offset-based) **Steps:**
...
**Objective:** Verify idempotency keys prevent duplicate operations **Steps:**
...
**Objective:** Verify API rate limits enforce request throttling **Steps:**
...
**Objective:** Verify 404 responses for non-existent resources **Steps:**
...
**Objective:** Verify RBAC prevents unauthorized actions **Steps:**
...
**Objective:** Verify API handles different content types **Steps:**

**Test 10a: JSON Request**

1. POST `/api/funds` Header: `Content-Type: application/json` Body: JSON payload
2. Verify response 201

**Test 10b: Unsupported Content Type**

1. POST `/api/funds` Header: `Content-Type: application/xml` Body: XML payload
...
**Objective:** Verify CORS headers allow frontend access **Steps:**

**Test 11a: Preflight Request (OPTIONS)**

1. OPTIONS `/api/funds` Header: `Origin: http://localhost:5173`
...
1. GET `/api/funds` Header: `Origin: http://localhost:5173`
2. Verify response 200
3. Verify header: `Access-Control-Allow-Origin: http://localhost:5173`

**Test 11c: Unauthorized Origin**

1. GET `/api/funds` Header: `Origin: http://malicious-site.com`
...
**Objective:** Verify API prevents SQL injection attacks **Steps:**
...
**Objective:** Verify API sanitizes inputs to prevent XSS **Steps:**
...
- [cheatsheets/api.md](../../cheatsheets/api.md) - API design patterns
- [cheatsheets/anti-pattern-prevention.md](../../cheatsheets/anti-pattern-prevention.md) -
