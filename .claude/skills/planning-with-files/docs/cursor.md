...
Create `.cursor/rules/planning-with-files.mdc` in your project with the content
from this repo.
...
> **Note:** Hooks (PreToolUse, PostToolUse, Stop, SessionStart) are **Claude
> Code specific** and will NOT work in Cursor.
...
The `.cursor/rules/planning-with-files.mdc` file contains all the planning
guidelines formatted for Cursor's rules system.
...
1. **Pin the planning files:** Keep task_plan.md open in a split view for easy
   reference.

2. **Add to .cursorrules:** You can also add planning guidelines to your
...
4. **Check status regularly:** Without the Stop hook, manually verify completion
   before finishing.
