...
- **Template Files Not Found in Cache** (Fixes #18)
  - Root cause: `${CLAUDE_PLUGIN_ROOT}` resolves to repo root, but templates
    were only in subfolders
...
- **Backward Compatible:** Yes (works with older Claude Code, but new hooks may
  not fire)
...
- Planning files now correctly created in project directory, not skill
  installation folder
...
- **Hooks Integration** (Claude Code 2.1.0+)
  - `PreToolUse` hook: Automatically reads `task_plan.md` before Write/Edit/Bash
    operations
