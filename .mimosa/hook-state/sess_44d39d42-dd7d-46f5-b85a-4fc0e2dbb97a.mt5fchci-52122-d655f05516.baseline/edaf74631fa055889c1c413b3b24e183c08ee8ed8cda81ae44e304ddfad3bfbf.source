...
test('collect HAR + console for fund setup step2→3', async ({ browser }) => {
  ...
  writeFileSync(
    'triage-output/console.txt',
    consoleLines.join('\n') || 'No console output captured'
  );
  writeFileSync('triage-output/errors.txt', errors.join('\n') || 'No errors captured');

  console.warn('✓ Triage data collected:');
  console.warn('  - network.har (all network traffic)');
  console.warn('  - console.txt (all console output)');
  console.warn('  - errors.txt (page errors and network failures)');
});
