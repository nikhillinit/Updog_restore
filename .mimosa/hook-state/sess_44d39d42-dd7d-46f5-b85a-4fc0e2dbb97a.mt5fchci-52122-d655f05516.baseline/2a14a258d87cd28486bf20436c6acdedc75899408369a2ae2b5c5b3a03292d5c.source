...
const _lpNps = new Gauge({
  name: 'lp_nps',
  help: 'LP Net Promoter Score',
  labelNames: ['lpId'],
});

const _dealVelocity = new Histogram({
...
export class LPBusinessMetrics {
  ...
  static async traceReportGeneration(reportId: string, _startTime: number) {
    ...
    type StageKey = 'dataFetch' | 'processing' | 'rendering' | 'storage';

    // Record stage timings
    const recordStage = (stage: StageKey, duration: number) => {
    ...
    return {
      ...
      complete: () => {
        ...
        const bottleneck = Object.entries(stages).reduce((a, b) => (b[1] > a[1] ? b : a));
  ...
  static monitorDataIntegrity(
    ...
    if (params.lpCount !== undefined && params.lpCount > 100) {
  ...
  private static async calculateEngagementScore(lpId: string): Promise<number> {
    ...
    const { active_days, total_actions } = result.rows[0]! as {
      active_days: number;
