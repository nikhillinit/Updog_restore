...
async function seedTestData(connectionString: string, options: SeedOptions = {}) {
  ...
  try {
      ...
      TEST_DATA.companies.map(async (companyData) => {
        ...
        const [investment] = await db
          ...
          .values({
            ...
            ownership: (
              (companyData.initialInvestment / companyData.initialValuation) *
              100
            ).toFixed(2),
            createdAt: new Date(),
            updatedAt: new Date(),
        ...
        console.log(
          `    - Initial investment: $${(companyData.initialInvestment / 1000000).toFixed(1)}M at $${(companyData.initialValuation / 1000000).toFixed(0)}M valuation`
        );
        ...
        if (!options.minimal && companyData.stage !== 'Seed') {
          await db.insert(investments).values({
            ...
            ownership: (
              ((companyData.initialInvestment * 0.5) / (companyData.initialValuation * 2)) *
              100
            ).toFixed(2),
            createdAt: new Date(),
            updatedAt: new Date(),
          ...
          console.log(
            `    - Follow-on investment: $${((companyData.initialInvestment * 0.5) / 1000000).toFixed(1)}M`
          );
        }
        ...
        if (companyData.status === 'exited' && companyData.exitValue) {
          ...
          console.log(
            `    - Exit: $${(companyData.exitValue / 1000000).toFixed(1)}M (${moic.toFixed(2)}x MOIC)`
          );
        }
    ...
    if (!options.minimal) {
        ...
        TEST_DATA.lpAccounts.map(async (lpData) => {
          ...
          await db.insert(lpFundCommitments).values({
            ...
            commitmentPercentage: (
              (lpData.commitment / parseInt(TEST_DATA.fund.size)) *
              100
            ).toFixed(4),
            status: 'active',
            createdAt: new Date(),
          ...
          console.log(
            `    - Commitment: $${(lpData.commitment / 1000000).toFixed(1)}M (${((lpData.commitment / parseInt(TEST_DATA.fund.size)) * 100).toFixed(2)}%)`
          );
          ...
          if (lpData.distributionsReceived > 0) {
            ...
            console.log(
              `    - Distributed: $${(lpData.distributionsReceived / 1000000).toFixed(2)}M`
            );
          }
      ...
      console.log(
        `[SUCCESS] Created ${lpAccounts.length} LP accounts with commitments and capital activity`
      );
    }
    ...
    console.log(
      `  - Investments: ${TEST_DATA.companies.length + (options.minimal ? 0 : TEST_DATA.companies.filter((c) => c.stage !== 'Seed').length)}`
    );
    if (!options.minimal) {
...
export async function seedDatabaseForLocalTarget(
  input: { databaseUrl?: string; env?: NodeJS.ProcessEnv; options?: SeedOptions } = {}
): Promise<void> {
  ...
  await seedTestData(databaseUrl, input.options);
}
