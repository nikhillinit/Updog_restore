...
test.describe('Basic Application Navigation', () => {
  ...
  test('should display navigation sidebar', async ({ page }) => {
    ...
    if (currentUrl.includes('/fund-setup')) {
      // SKIP: sidebar navigation assertions require a post-setup route instead of the fund-setup flow
      test.skip();
    }
  ...
  test('should be responsive on mobile devices', async ({ page }) => {
    ...
    const mobileMenuToggle = page
      .locator('button[aria-label*="menu" i], .menu-toggle, [data-testid*="menu-toggle"]')
      .first();
