...
describe('API Funds Contract', () => {
  test('GET /api/funds matches contract & includes UI edge-cases', async () => {
    ...
    setOptionalEnv('ENABLE_API_STUB', 'true');
    ...
    if (process.env.ENABLE_API_STUB === 'true') {
      ...
      expect(funds.map((f: any) => f.currency)).toEqual(expect.arrayContaining(['USD', 'EUR']));
  ...
  test('stub disabled returns graceful error', async () => {
    ...
    setOptionalEnv('ENABLE_API_STUB', 'false');
    ...
    setOptionalEnv('ENABLE_API_STUB', originalEnv);
  });
});
