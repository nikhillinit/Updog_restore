...
describe('validateWizardPortfolio', () => {
  ...
  it('should reject negative invested amounts', () => {
    const portfolio = [createMockCompany({ name: 'Test Co', investedAmount: -100000 })];
  ...
  it('should reject invalid ownership percentages (>100%)', () => {
    const portfolio = [createMockCompany({ name: 'Test Co', ownershipPercent: 150 })];

    const result = validateWizardPortfolio(portfolio);

    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.includes('Ownership must be between 0% and 100%'))).toBe(
      true
    );
  });
  ...
  it('should reject companies with missing ID', () => {
    const portfolio = [createMockCompany({ id: '', name: 'Test Co' })];
  ...
  it('should reject companies with missing name', () => {
    const portfolio = [createMockCompany({ id: 'c1', name: '' })];
...
describe('enrichWizardMetrics', () => {
  ...
  it('should throw error if generalInfo is missing', () => {
    ...
    expect(() => enrichWizardMetrics(mockAllocation, context)).toThrow(
      'Required wizard context not available'
    );
  });
...
describe('Edge Cases & Error Handling', () => {
  it('should handle portfolio with zero invested amounts gracefully', () => {
    const portfolio = [createMockCompany({ investedAmount: 0, currentValuation: 1000000 })];
  ...
  it('should handle portfolio with negative valuations', () => {
    const portfolio = [createMockCompany({ investedAmount: 1000000, currentValuation: -500000 })];

    const result = validateWizardPortfolio(portfolio);

    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.includes('Current valuation cannot be negative'))).toBe(
      true
    );
  });
});
