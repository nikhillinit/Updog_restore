...
const tier = (
  tierArg ? tierArg.split('=')[1] : process.env.AI_REVIEW_TIER || 'cloud'
).toLowerCase();
if (!packageName) {
  console.error(
    'Usage: npm run ai:review -- --package=<name> [--tier=mock|local|hf|cloud|all|health]'
  );
  process.exit(2);
}
...
function resolvePackageRoot(name: string): string {
  const candidates = [`client`, `server`, `shared`, `scripts/${name}`, `tools/${name}`, name];
...
function pickProviders(t: string): ProviderConfig[] {
  ...
  const hf: ProviderConfig[] = [
    { id: 'hf:deepseek-coder-33b', role: 'Code Analysis', costCapUsd: 1.0, enabled: true },
    { id: 'hf:codellama-34b', role: 'Code Refactor', costCapUsd: 1.0, enabled: true },
  ];
  ...
  switch (t) {
    case 'mock':
      return cloud.length
        ? cloud
        : [{ id: 'gpt4', role: 'Architecture Review', costCapUsd: 1, enabled: true }]; // mocked if no keys
    case 'local':
      return local;
    case 'hf':
      return hf;
    case 'health':
      return [{ id: 'deepseek', role: 'Health Check', costCapUsd: 0.25, enabled: true }];
    case 'all':
      return [...local, ...hf, ...cloud];
    case 'cloud':
    default:
      return cloud.length
        ? cloud
        : [{ id: 'gpt4', role: 'Architecture Review', costCapUsd: 1, enabled: true }];
  }
}
...
(async () => {
  ...
  for (const r of results) {
    ...
    const filename =
      plan.outputs.files[key as keyof typeof plan.outputs.files] ??
      `${r.providerId.toUpperCase().replace(/[^A-Z0-9._-]+/g, '_')}_REVIEW.md`;
    const outPath = resolve(reviewsDir, filename);
    writeFileSync(outPath, r.report, 'utf-8');
    console.log(
      `✅ ${r.providerId.toUpperCase()} Complete → ${outPath.replace(process.cwd() + '/', '')}`
    );
  }
