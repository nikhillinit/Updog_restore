...
export const StageSchema = z.enum(['seed', 'series_a', 'series_b', 'series_c', 'growth']);
export type Stage = z.infer<typeof StageSchema>;

export const StageAllocationSchema = z.object({
  stage: StageSchema,
  allocationPct: z.number().min(0).max(1).describe('Allocation percentage (0.0 to 1.0)'),
});
...
export const FundModelInputsSchema = z
  .object({
    // Fund basics
    fundSize: z.number().positive().describe('Total committed capital in dollars'),

    periodLengthMonths: z
      .number()
      .int()
      .positive()
    ...
    graduationRates: z
      .record(StageSchema, z.number().min(0).max(1))
    ...
    monthsToGraduate: z
      .record(StageSchema, z.number().int().positive())
  ...
  .superRefine((inputs, ctx) => {
    ...
    // =====================
    inputs.stageAllocations.forEach((stage, _idx) => {
    ...
    // =====================
    const maxManagementFees =
      inputs.fundSize * inputs.managementFeeRate * inputs.managementFeeYears;
    ...
    if (estimatedReserveNeed > reservePool * 1.2) {
      // 20% buffer
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const PeriodResultSchema = z.object({
  ...
  periodStart: z.string().datetime().describe('ISO 8601 date string'),
  periodEnd: z.string().datetime().describe('ISO 8601 date string'),

  // Cash flows (all fields required for invariants)
  contributions: z.number().nonnegative().describe('Capital called from LPs this period'),

  investments: z.number().nonnegative().describe('Capital deployed into companies this period'),

  managementFees: z.number().nonnegative().describe('Management fees paid this period'),

  exitProceeds: z.number().nonnegative().describe('Cash received from company exits this period'),

  distributions: z.number().nonnegative().describe('Cash distributed to LPs this period'),

  unrealizedPnl: z.number().describe('Mark-to-market gains/losses (can be negative)'),

  // Ending balances
  nav: z.number().nonnegative().describe('Net Asset Value end-of-period'),
  ...
  tvpi: z
    ...
    .nonnegative()
    .describe(
      'Total Value to Paid-In: (cumulative distributions + NAV) / cumulative contributions'
    ),
  ...
  irrAnnualized: z.number().describe('Internal Rate of Return (XIRR, annualized %)'),
});
...
export const FundModelOutputsSchema = z.object({
  ...
  kpis: z.object({
    tvpi: z
      ...
      .nonnegative()
      .describe(
        'Total Value to Paid-In: (cumulative distributions + ending NAV) / cumulative contributions'
      ),
