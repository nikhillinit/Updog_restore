...
describe('facts snapshot fund ownership', () => {
  it('accepts a facts snapshot found inside the authoritative fund scope', async () => {
    await expect(
      assertOwnedByFund({
        db: ownershipDatabase([{ id: 51 }]),
        fundId: 7,
        ref: { kind: 'facts_snapshot', id: 51 },
      })
    ).resolves.toBeUndefined();
  });

  it('rejects a facts snapshot absent from the authoritative fund scope', async () => {
    await expect(
      assertOwnedByFund({
