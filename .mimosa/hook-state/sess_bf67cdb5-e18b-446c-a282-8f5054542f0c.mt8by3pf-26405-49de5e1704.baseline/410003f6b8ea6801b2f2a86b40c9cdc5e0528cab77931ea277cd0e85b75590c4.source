...
describe('financing event fund ownership', () => {
  it('accepts a financing event found inside the authoritative fund scope', async () => {
    await expect(
      assertOwnedByFund({
        db: ownershipDatabase([{ id: 51 }]),
        fundId: 7,
        ref: { kind: 'financing_event', id: 51 },
      })
    ).resolves.toBeUndefined();
  });

  it('rejects a financing event absent from the authoritative fund scope', async () => {
    await expect(
      assertOwnedByFund({
