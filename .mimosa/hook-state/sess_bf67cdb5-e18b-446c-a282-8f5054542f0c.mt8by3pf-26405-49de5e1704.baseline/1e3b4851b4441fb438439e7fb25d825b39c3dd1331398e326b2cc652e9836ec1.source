...
describe('participation fund ownership', () => {
  it('accepts a participation found inside the authoritative fund scope', async () => {
    await expect(
      assertOwnedByFund({
        db: ownershipDatabase([{ id: 51 }]),
        fundId: 7,
        ref: { kind: 'participation', id: 51 },
      })
    ).resolves.toBeUndefined();
  });

  it('rejects a participation absent from the authoritative fund scope', async () => {
    await expect(
      assertOwnedByFund({
