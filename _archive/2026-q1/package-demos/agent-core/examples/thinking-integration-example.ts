...
class TestRepairAgentWithThinking extends withThinking(BaseAgent)<TestFailure, RepairStrategy> {
  ...
  private calculateConfidence(thinking: string[]): number {
    ...
    const hasCodeExamples = thinking.some(
      (block) => block.includes('```') || block.includes('function') || block.includes('const')
    );
...
export { TestRepairAgentWithThinking, CodeReviewerWithThinking, runThinkingIntegrationDemo };
