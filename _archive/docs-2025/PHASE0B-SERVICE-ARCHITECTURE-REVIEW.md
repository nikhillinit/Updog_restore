...
Architectural recommendations for SnapshotService and LotService implementation

---

## Executive Summary

**RECOMMENDATION:** Implement services with direct Drizzle ORM access (no
repository pattern) and centralized transaction wrappers. This aligns with
existing codebase patterns and leverages PostgreSQL's ACID guarantees while
...
**Architecture Decision:** Single-responsibility services with composition (not
inheritance).
...
**Finding:** Services use **direct Drizzle ORM access** with circuit breaker
wrappers
...
**Recommendation:** Services can rely on connection-level circuit breaker
protection.
...
**Recommendation:** Services should NOT duplicate idempotency logic. Trust
middleware.

### 2.2 Database Idempotency Pattern

**Finding:** Database has **scoped idempotency indexes** for data-level
deduplication
...
2. **Database (PostgreSQL):** Prevents duplicate records (permanent: scoped by
   parent)
...
**Recommendation:** Services use `onConflictDoNothing()` for safe inserts with
idempotency keys.
...
**Exception:** If you need **shared utilities**, create a **separate utility
module**:
...
**ANSWER: Middleware handles API idempotency, Services use database
constraints**
...
class SnapshotService {
  async create(data: CreateSnapshotData): Promise<ForecastSnapshot> {
    ...
    const fund = await db.select().from(funds).where(eq(funds.id, data.fundId));
...
**Recommendation:** Services should validate parent entities exist before
creation.
...
**Security Note:** Always verify parent entity belongs to fund (prevent
cross-tenant access).
...
describe('SnapshotService - Transaction Safety', () => {
  it('rolls back on error during state capture', async () => {
    ...
    vi.spyOn(service as any, 'captureMetricsState').mockRejectedValue(
    ...
    await expect(service.captureState(snapshot.id)).rejects.toThrow(
      'Metrics API down'
    );
