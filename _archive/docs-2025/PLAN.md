...
**Goal:** Implement cohort analysis to visualize portfolio performance by
vintage/sector with company-level and investment-level views.

**Design Reference:**
[docs/plans/2026-01-04-advanced-cohort-analysis-design.md](docs/plans/2026-01-04-advanced-cohort-analysis-design.md)
...
- [ ] Define `sectorMappings` table with:
  - PK: id (uuid)
  - Fields: fundId, taxonomyVersion, rawValue, rawValueNormalized,
    canonicalSectorId, confidenceScore, source
  - Unique index: (fundId, taxonomyVersion, rawValueNormalized)
- [ ] Define `companyOverrides` table with:
  - PK: id (uuid)
  - Fields: fundId, taxonomyVersion, companyId, canonicalSectorId (nullable),
    excludeFromCohorts
  - Unique index: (fundId, taxonomyVersion, companyId)
- [ ] Define `investmentOverrides` table with:
  - PK: id (uuid)
  - Fields: fundId, investmentId, excludeFromCohorts, vintageYear,
    vintageQuarter
  - Unique index: (fundId, investmentId)
- [ ] Define `cohortDefinitions` table with:
  - PK: id (uuid)
  - Fields: fundId, name, vintageGranularity, sectorTaxonomyVersion, unit,
...
**Success Criteria:** Seed function populates all tables correctly, can run
...
**Success Criteria:** Pipeline produces correct CohortAnalyzeResponse, backward
compatible
