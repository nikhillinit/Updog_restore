...
describe('Variance Tracking Database Schema', () => {
  ...
  describe('fund_baselines table', () => {
    it('should create baseline with all required fields', async () => {
      ...
      `,
        [
          ...
          baseline.deployed_capital,
          baseline.portfolio_count,
          baseline.created_by,
        ]
      );
    ...
    it('should enforce baseline_type enum constraint', async () => {
      ...
      await expect(
        ...
        `,
          [
            invalidBaseline.fund_id,
            invalidBaseline.name,
            invalidBaseline.baseline_type,
            invalidBaseline.period_start,
            invalidBaseline.period_end,
            invalidBaseline.snapshot_date,
            invalidBaseline.total_value,
            invalidBaseline.deployed_capital,
            invalidBaseline.portfolio_count,
    ...
    it('should enforce period_end > period_start constraint', async () => {
      ...
      await expect(
        ...
        `,
          [
            invalidPeriodBaseline.fund_id,
            invalidPeriodBaseline.name,
            invalidPeriodBaseline.baseline_type,
            invalidPeriodBaseline.period_start,
            invalidPeriodBaseline.period_end,
            invalidPeriodBaseline.snapshot_date,
            invalidPeriodBaseline.total_value,
            invalidPeriodBaseline.deployed_capital,
            invalidPeriodBaseline.portfolio_count,
    ...
    it('should create performance alert with all required fields', async () => {
      ...
      `,
        [
          ...
          'performance',
          'IRR Decline Alert',
          'IRR has declined by more than 5%',
          'irr',
  ...
  describe('alert_rules table', () => {
    it('should create alert rule with all required fields', async () => {
      const [result] = await db.execute(
        ...
        RETURNING id
      `,
        ['IRR Threshold Rule', 'threshold', 'irr', 'lt', -0.05, 'critical', 'performance', 1]
      );
    ...
    it('should enforce valid rule_type enum', async () => {
      await expect(
        ...
        `,
          ['Invalid Rule', 'invalid_type', 'irr', 'lt', -0.05, 'critical', 'performance', 1]
        )
      ).rejects.toThrow();
    });
    ...
    it('should enforce secondary_threshold requirement for between operator', async () => {
      ...
      await expect(
        ...
        `,
          ['Between Rule', 'threshold', 'irr', 'between', 0.1, 'warning', 'performance', 1]
        )
      ).rejects.toThrow();
      ...
      const [result] = await db.execute(
        ...
        RETURNING id
      `,
        ['Valid Between Rule', 'threshold', 'irr', 'between', 0.1, 0.2, 'warning', 'performance', 1]
      );
  ...
  describe('Database views', () => {
    ...
    it('should query variance_summary view', async () => {
      ...
      const [baseline] = await db.execute(
        'SELECT id FROM fund_baselines WHERE fund_id = 1 LIMIT 1'
      );
