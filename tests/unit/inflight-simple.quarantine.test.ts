...
describe('In-flight Request Tracking', () => {
  ...
  class InflightRegistry {
    ...
    add(hash: string, promise: Promise<unknown>): void {
  ...
  it('should track requests while in-flight', async () => {
    ...
    // The finally block runs in a microtask after the promise resolves
    // We need to wait for the microtask queue to be processed
  ...
  it('should namespace hashes by environment', () => {
    function computeHash(env: string, payload: Record<string, unknown>): string {
      let hash = 0x811c9dc5;
