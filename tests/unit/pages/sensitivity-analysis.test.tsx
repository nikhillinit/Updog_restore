...
describe('SensitivityAnalysisPage', () => {
  it('renders the live Monte Carlo workspace and exposes the one-way tab as enabled', () => {
    render(<SensitivityAnalysisPage />);

    expect(screen.getByText('Sensitivity Analysis')).toBeInTheDocument();
    expect(
      screen.getByText(/Monte Carlo backtesting and one-way parameter sensitivity are live/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
