...
const LP_REPORTING_ROUTE_POLICY_KEYS = [
  ...
  'POST /api/funds/:fundId/imports/batches',
...
const LP_REPORTING_ADDITIONAL_POLICY_GROUPS: ReadonlyArray<{
  ...
  {
    keys: ['POST /api/funds/:fundId/imports/batches'],
    expected: {
  ...
  {
    keys: ['POST /api/funds/:fundId/imports/batches/:batchId/commit'],
    expected: {
...
describe('route policy coverage', () => {
  ...
  it('classifies internal-economics receipt reads as investment-team-only immutable lineage', () => {
    const policy = expectPolicy('GET /api/funds/:fundId/internal-economics/runs/:runId');
    ...
    expect(policy.fundScopeMode).toBe('route_param_fund_id');
    expect(policy.workflowRequirement).toBe('immutable_run_receipt_read');
    expect(policy.exportPolicy).toBe('not_exportable');
    expect(policy.provenanceRequired).toBe(true);
    expect(policy.staleBlocksExport).toBe(false);
    expect(policy.humanReviewRequired).toBe(true);
  ...
  it('covers every LP-reporting metric-run and import route', () => {
    expect(LP_REPORTING_ROUTE_POLICY_KEYS).toHaveLength(43);
