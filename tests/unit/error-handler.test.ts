...
describe('Error Handler Request-ID', () => {
  ...
  it('should generate server X-Request-ID and optionally preserve client ID', async () => {
    ...
    const response = await request(app).get('/test-error').set('X-Request-ID', clientId);

    expect(response.status).toBe(400);
    // Server generates its own ID (security: prevent log injection)
    expect(response.headers['x-request-id']).toMatch(/^req_[a-f0-9-]+$/);
  ...
  it('should preserve server-generated X-Request-ID when already set', async () => {
    ...
    // Server generates its own ID (security), client ID ignored
    expect(response.headers['x-request-id']).toMatch(/^req_[a-f0-9-]+$/);
    expect(response.headers['x-request-id']).not.toBe('client-partial-123');
    expect(response.body.error).toBe('Partial error');

    // Client ID preserved separately in non-production
    if (process.env.NODE_ENV !== 'production') {
      expect(response.headers['x-client-request-id']).toBe('client-partial-123');
    }
  });
  ...
  it('should handle errors with custom status codes', async () => {
    ...
    const response = await request(testApp).get('/test-custom');
  ...
  it('should log errors with request context when logger exists', async () => {
    ...
    const response = await request(testApp).get('/test-logged');
