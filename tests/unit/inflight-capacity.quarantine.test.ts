...
const { startCreateFund, isCreateFundInFlight, cancelCreateFund, computeCreateFundHash } =
...
describe('In-flight Capacity Management', () => {
  ...
  it('should track in-flight requests', async () => {
    ...
    vi.mocked(global.fetch).mockReturnValue(deferredRequest.promise);
  ...
  it('should deduplicate concurrent identical requests', async () => {
    ...
    const payload = { name: 'Test Fund', size: 1000000 };
    const finalizedPayload = { ...payload, modelVersion: 'reserves-ev1' };
    const _hash = computeCreateFundHash(finalizedPayload);
  ...
  it('should support manual cancellation', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation((_url: string, options?: RequestInit) => {
      return new Promise((resolve, reject) => {
        ...
        // Never resolve normally - this simulates a long-running request
      });
    });
