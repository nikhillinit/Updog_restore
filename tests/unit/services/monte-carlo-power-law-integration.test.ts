...
describe.skip('Monte Carlo Power Law Integration', () => {
  ...
  describe('70% Failure Rate Validation', () => {
    ...
    it('should show different failure rates by stage', async () => {
      ...
      const seedFailureRate =
        seedForecast.multiple.scenarios.filter((m) => m <= 1.0).length /
        seedForecast.multiple.scenarios.length;
  ...
  describe('Series A Chasm Validation', () => {
    it('should properly reflect Series A Chasm graduation rates', async () => {
      ...
      if (stagePerformance.seed && stagePerformance['series-a']) {
        ...
        expect(stagePerformance.seed.standardDeviation).toBeGreaterThan(
          stagePerformance['series-a'].standardDeviation
        );
      }
    ...
    it('should show increasing unicorn rates in later stages', async () => {
      ...
      const seedUnicornRate =
        seedForecast.multiple.scenarios.filter((m) => m > 50).length / scenarios;
  ...
  describe('Risk Metrics Integration', () => {
    it('should calculate realistic risk metrics with power law returns', async () => {
      ...
      expect(forecast.riskMetrics.valueAtRisk[5]).toBeLessThan(
        forecast.riskMetrics.valueAtRisk[10]
      );
      expect(forecast.riskMetrics.valueAtRisk[10]).toBeLessThan(
        forecast.riskMetrics.valueAtRisk[25]
      );
