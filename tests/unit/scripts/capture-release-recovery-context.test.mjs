...
describe('capture-release-recovery-context', () => {
  ...
  it('captures only normalized provider identity through read-only API requests', { retry: 0 }, async () => {
  ...
  it('rejects credential-bearing Vercel provider URLs before serializing context', { retry: 0 }, async () => {
    await expect(captureProviderBaseline({
      baselineMainSha: BASELINE_MAIN_SHA,
      plannedPrHeadSha: PLANNED_PR_HEAD_SHA,
      planSha256: PLAN_SHA256,
      environment: providerEnvironment(),
      fetchImpl: providerFetch({ deploymentUrl: 'https://user:password@baseline.vercel.app/?token=secret' }),
    })).rejects.toThrow(/release recovery context/i);
  });

  it('proves exact current main, PR head, and immutable plan content before capture', { retry: 0 }, async () => {
    ...
    const fetchImpl = async (url) => {
      ...
      if (url.endsWith('/pulls/1385')) return { ok: true, json: async () => ({ head: { sha: PLANNED_PR_HEAD_SHA } }) };
      throw new Error('unexpected URL');
    };
    const execFileImpl = async (_command, args) => {
      const key = args.join(' ');
      if (key === 'rev-parse HEAD') return { stdout: `${BASELINE_MAIN_SHA}\n` };
      if (key === 'fetch --no-tags origin pull/1385/head:refs/remotes/origin/pr-1385') return { stdout: '' };
  ...
  it('rejects unknown fields, response bodies, secret-shaped keys and values, and credential URLs', { retry: 0 }, () => {
    for (const input of [
      { ...captureInput(), extra: true },
      { ...captureInput(), providerIdentity: { ...captureInput().providerIdentity, responseBody: {} } },
      { ...captureInput(), providerIdentity: { ...captureInput().providerIdentity, apiToken: 'secret' } },
      { ...captureInput(), providerIdentity: { ...captureInput().providerIdentity, vercel: { ...captureInput().providerIdentity.vercel, hostname: 'https://user:password@production.example.com/?token=secret' } } },
  ...
  it('rejects malformed IDs and SHAs plus expected provider identity mismatches', { retry: 0 }, () => {
    for (const input of [
      { ...captureInput(), baselineMainSha: 'A'.repeat(40) },
      { ...captureInput(), githubRunId: '0' },
      { ...captureInput(), providerIdentity: { ...captureInput().providerIdentity, vercel: { ...captureInput().providerIdentity.vercel, deploymentId: 'bad id' } } },
      { ...captureInput(), providerIdentity: { ...captureInput().providerIdentity, railway: { ...captureInput().providerIdentity.railway, projectId: 'wrong-project' } } },
    ]) {
      expect(() => buildReleaseRecoveryContext(input)).toThrow(/release recovery context/i);
    }
  });

  it('writes the context with mode 0600 and never serializes expected identity or raw response data', { retry: 0 }, async () => {
    ...
    try {
      await (await import('node:fs/promises')).writeFile(inputPath, JSON.stringify(captureInput()), 'utf8');
