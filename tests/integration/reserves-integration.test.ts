...
describe.skip('Reserves v1.1 Integration Tests', () => {
  ...
  describe('Production-Scale Scenarios', () => {
    ...
    it('should maintain consistency across multiple calculation runs', async () => {
      ...
      const results = await Promise.all(
        Array(5)
          .fill(null)
          .map(() => calculateReservesSafe(input, config))
      );
      ...
      for (let i = 1; i < results.length; i++) {
        ...
        expect(results[i].data!.metadata.total_allocated_cents).toBe(
          firstResult.data!.metadata.total_allocated_cents
        );
  ...
  describe('Shadow Intelligence Integration', () => {
    it('should detect and categorize divergences correctly', async () => {
      ...
      expect(analysis.divergenceType).toBeOneOf([
        'logic',
        'improvement',
        'rounding',
        'data',
