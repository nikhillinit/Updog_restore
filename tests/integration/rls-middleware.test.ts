 ...
 *
 * @group integration
 * FIXME: These tests require live database + proper JWT configuration.
 * Root cause: Tests fail with "invalid signature" and database connection issues.
 * Mock setup is incomplete - needs proper JWT_SECRET and database mock coordination.
 * Skipping until integration test environment is properly configured.
 */
...
import {
  withRLSTransaction,
  verifyRLSContext,
} from '../../server/middleware/with-rls-transaction.js';
...
describe.skip('RLS Middleware Integration', () => {
  ...
  describe('RLS Transaction Scoping', () => {
    it('should set RLS context in transaction', async () => {
      ...
      app.get('/test/rls', requireSecureContext, withRLSTransaction(), async (req: any, res) => {
    ...
    it('should rollback transaction on error', async () => {
      ...
      await request(app).get('/test/rls-error').set('Authorization', `Bearer ${token}`).expect(500);
  ...
  describe('Concurrency Control', () => {
    it('should require If-Match header when configured', async () => {
      app.post('/test/if-match', requireIfMatch(), (req, res) => {
        res.json({ success: true });
      });

      const response = await request(app).post('/test/if-match').send({ data: 'test' }).expect(428);
    ...
    it('should process request with valid If-Match header', async () => {
      app.post('/test/if-match-valid', requireIfMatch(), (req: any, res) => {
        res.json({
          success: true,
          ifMatch: req.ifMatch,
        });
