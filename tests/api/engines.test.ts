...
describe('API Routes Integration', () => {
    ...
    if (url.includes('/api/reserves/')) {
      return Promise.resolve({
        ...
        json: () => Promise.resolve([
          { allocation: 1500000, confidence: 0.3, rationale: 'Rule-based allocation (cold-start mode)' },
          { allocation: 1000000, confidence: 0.3, rationale: 'Rule-based allocation (cold-start mode)' },
          { allocation: 2400000, confidence: 0.3, rationale: 'Rule-based allocation (cold-start mode)' }
        ])
      });
    }
  ...
  it('should validate reserve engine API response structure', async () => {
    ...
    expect(data[0]).toHaveProperty('allocation');
    expect(data[0]).toHaveProperty('confidence');
    expect(data[0]).toHaveProperty('rationale');
  });
  ...
  it('should validate pacing engine API response structure', async () => {
    ...
    expect(Array.isArray(data)).toBe(true);
    expect(data[0]).toHaveProperty('quarter');
    expect(data[0]).toHaveProperty('deployment');
