...
function logProductionSmoke(...args: unknown[]): void {
  if (process.env.PROD_SMOKE_DEBUG === 'true') {
    console.warn(...args);
  }
}
...
test.describe('Production Smoke Tests', () => {
  ...
  test('should verify production health endpoint', async ({ request }) => {
    ...
    logProductionSmoke('Production health check passed:', healthData);
  });
  ...
  test('should verify production assets load correctly', async ({ page }) => {
    ...
    const criticalFailures = failedRequests.filter(
      (url) => url.endsWith('.js') || url.endsWith('.css') || url.endsWith('.ico')
    );

    expect(criticalFailures).toHaveLength(0);

    if (failedRequests.length > 0) {
      logProductionSmoke('Non-critical failed requests:', failedRequests);
    }
  });
  ...
  test('should verify SSL certificate and security headers', async ({ request }) => {
    ...
    if (frameOptions || csp) {
      expect(
        frameOptions === 'DENY' || frameOptions === 'SAMEORIGIN' || csp?.includes('frame-ancestors')
      ).toBeTruthy();
    }

    logProductionSmoke('Security headers check passed');
  });
  ...
  test('should verify database connectivity in production', async ({ page }) => {
    ...
    logProductionSmoke('Database connectivity check passed');
  });
  ...
  test('should handle high traffic gracefully', async ({ page }) => {
    ...
    logProductionSmoke(
      `Concurrent requests completed in ${loadTime}ms, ${successfulResponses.length}/5 successful`
    );
  });
  ...
  if (previewUrl && previewUrl !== prodUrl) {
    test('should verify preview environment matches production', async ({ page }) => {
      ...
      logProductionSmoke('Preview environment verification passed');
    });
  }
    ...
    for (const endpoint of apiEndpoints) {
      try {
        ...
        if (response.status() === 200) {
          logProductionSmoke(`API endpoint ${endpoint} is accessible`);
        ...
        logProductionSmoke(`API endpoint ${endpoint} not accessible: ${error}`);
      }
    }
  });
  ...
  test('should verify CDN and static assets performance', async ({ page }) => {
    ...
    const slowAssets = assetRequests.filter(
      (req) => req.timing && req.timing.responseEnd > 5000 // 5 seconds
    );
    ...
    logProductionSmoke(
      `Asset performance: ${assetRequests.length} assets, ${failedAssets.length} failed, ${slowAssets.length} slow`
    );
  });
  ...
  test('should verify cross-origin resource sharing', async ({ request }) => {
    ...
    if (corsHeaders.accessControl) {
      expect(
        corsHeaders.accessControl === '*' || corsHeaders.accessControl.includes('updog')
      ).toBeTruthy();
    }

    logProductionSmoke('CORS configuration verified:', corsHeaders);
  });
  ...
  test('should verify monitoring and analytics are working', async ({ page }) => {
    ...
    if (monitoringCalls.length > 0) {
      logProductionSmoke('Monitoring services detected:', monitoringCalls.length);
    } else {
      logProductionSmoke('No monitoring services detected (this may be intentional)');
    }
