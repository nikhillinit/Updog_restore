...
function logE2ETestHelper(...args: unknown[]): void {
  if (process.env.E2E_HELPER_DEBUG === 'true') {
    console.warn(...args);
  }
}
...
export class TestHelpers {
  ...
  static async waitForElementSafely(
    ...
    try {
      ...
      logE2ETestHelper(`Element not found: ${selector}. Error: ${String(error)}`);
      return false;
    }
  }
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (i < maxRetries - 1) {
          ...
          logE2ETestHelper(`Attempt ${i + 1} failed, retrying in ${delay}ms...`);
  ...
  static async clickSafely(
    ...
    try {
      ...
      logE2ETestHelper(`Click failed for ${selector}: ${String(error)}`);
      return false;
    }
  }
   ...
   */
  static async handleAuthenticationState(
    page: Page
  ): Promise<'authenticated' | 'unauthenticated' | 'setup-required'> {
   ...
   */
  static async testResponsiveBreakpoints(page: Page, test: () => Promise<void>): Promise<void> {
    const breakpoints = [
  ...
  static async verifyNoConsoleErrors(page: Page, allowedErrors: string[] = []): Promise<void> {
    ...
    if (errors.length > 0) {
      logE2ETestHelper('Console errors detected:', errors);
  ...
  static skipIf(condition: boolean, reason: string): void {
    if (condition) {
      logE2ETestHelper(`Skipping test: ${reason}`);
