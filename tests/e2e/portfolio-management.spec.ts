...
function logPortfolioE2E(...args: unknown[]): void {
  if (process.env.PORTFOLIO_E2E_DEBUG === 'true') {
    console.warn(...args);
  }
}
...
test.describe('Portfolio Management', () => {
  ...
  test.beforeEach(async ({ page, fund }) => {
    ...
    logPortfolioE2E(`Testing with fund: ${fund.fundName} (id: ${fund.fundId})`);
    ...
    // SKIP: portfolio e2e flow requires an authenticated session instead of a login redirect
  ...
  test('should display investments list correctly', async () => {
    ...
    if (investmentsCount > 0) {
      ...
      logPortfolioE2E('No investments found - testing empty state');
      await portfolioPage.verifyEmptyPortfolio();
    }
  });
  ...
  test('should export portfolio data', async () => {
    ...
    if (await portfolioPage.exportButton.isVisible()) {
      ...
      const downloadPromise = portfolioPage.page
      ...
      if (download) {
        ...
        const hasModal = await portfolioPage.page.locator('[role="dialog"], .modal').isVisible();
        const hasMessage = await portfolioPage.page
          .locator('.notification, .toast, .alert')
          .isVisible();
  ...
  test('should handle errors gracefully', async () => {
    ...
    for (const action of actions) {
      try {
        ...
        const hasErrorMessage = await portfolioPage.page
  ...
  test('should maintain data consistency', async () => {
    ...
    const portfolioHasValues = Object.values(portfolioMetrics).some(
      (v) => v !== 'N/A' && v !== '$0' && v.length > 0
    );
    const dashboardHasValues = Object.values(dashboardMetrics).some(
      (v) => v !== '$0' && v.length > 0
    );
