...
test.describe('Navigation and Routing', () => {
  ...
  test('should navigate between all main modules', async () => {
    ...
    for (const navTest of navigationTests) {
      try {
        ...
        console.error(`${navTest.name} navigation not available: ${error}`);
      }
    }
  });
  ...
  test('should handle direct URL navigation', async () => {
    ...
    for (const route of routes) {
      try {
        ...
        console.error(`Direct navigation to ${route} failed: ${error}`);
      }
    }
  });
  ...
  test('should show proper loading states', async () => {
    ...
    if (await navigationPage.portfolioLink.isVisible()) {
      ...
      const loadingElements = navigationPage.page.locator(
        '.loading, .spinner, .animate-spin, [data-testid="loading"]'
      );
