...
function logUserAuthE2E(...args: unknown[]): void {
  if (process.env.USER_AUTH_E2E_DEBUG === 'true') {
    console.warn(...args);
  }
}
...
test.describe('User Authentication Flow', () => {
  ...
  test('should successfully login with valid credentials', async () => {
    ...
    if (!hasError) {
      ...
      logUserAuthE2E('Demo credentials not configured - this is expected in test environment');
    }
  });
  ...
  test('should handle user registration flow', async () => {
    ...
    // SKIP: registration coverage depends on the target environment exposing a registration form
  ...
  test('should validate registration form fields', async () => {
    ...
    // Skip if registration form not available (demo mode)
    const hasRegisterForm = await authPage.registerForm.isVisible();
    // SKIP: registration validation coverage depends on the target environment exposing a registration form
    test.skip(!hasRegisterForm, 'Registration form not available');
  ...
  test('should show forgot password functionality', async () => {
    ...
    if (await authPage.forgotPasswordLink.isVisible()) {
      ...
      const currentUrl = await authPage.page.url();
      const onForgotPassword = currentUrl.includes('/forgot') || currentUrl.includes('/reset');
  ...
  test('should handle loading states during authentication', async () => {
    ...
    await authPage.loadingSpinner.isVisible();
    ...
    const formStillInteractive =
      (await authPage.loginButton.isEnabled()) ||
