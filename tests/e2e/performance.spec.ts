...
function logE2EPerformance(...args: unknown[]): void {
  if (process.env.E2E_PERF_DEBUG === 'true') {
    console.warn(...args);
  }
}
...
test.describe('Performance Tests', () => {
  test.beforeEach(async ({ page, fund }) => {
    ...
    logE2EPerformance(`Testing with fund: ${fund.fundName} (id: ${fund.fundId})`);
  ...
  test('should load homepage within performance budget', async ({ page }) => {
    ...
    logE2EPerformance(`Homepage load time: ${loadTime}ms`);
  });
  ...
  test('should load dashboard within performance budget', async ({ page }) => {
    ...
    // SKIP: performance assertions are invalid when auth or fund-setup redirects replace the dashboard
    test.skip(
    ...
    logE2EPerformance(`Dashboard load time: ${loadTime}ms`);
  });
  ...
  test('should have acceptable Core Web Vitals', async ({ page }) => {
    ...
    // SKIP: Core Web Vitals only apply to the target page, not redirect destinations
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    logE2EPerformance('Core Web Vitals:', vitals);
  ...
  test('should not have memory leaks on navigation', async ({ page }) => {
    ...
    const currentUrl = page.url();
    // SKIP: memory measurements are not meaningful when the app redirects away from the dashboard
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    for (let i = 0; i < 3; i++) {
      for (const route of navigationRoutes) {
        try {
          ...
          logE2EPerformance(`Navigation to ${route} failed: ${error}`);
        }
      }
    }
    ...
    logE2EPerformance(`Memory usage - Initial: ${initialMemory}, Final: ${finalMemory}`);
  ...
  test('should handle large data sets efficiently', async ({ page }) => {
    ...
    const currentUrl = page.url();
    // SKIP: scrolling benchmarks require the portfolio page, not a redirect target
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    const scrollableElements = await page
      .locator('.portfolio-list, .investments-table, [role="table"]')
      .all();
    ...
    logE2EPerformance(`Scrolling performance: ${scrollTime}ms`);
  });
  ...
  test('should load images efficiently', async ({ page }) => {
    ...
    const currentUrl = page.url();
    // SKIP: image timing checks only apply when the dashboard is the rendered page
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    if (images.length > 0) {
      ...
      const avgLoadTime =
        successfulLoads.reduce((sum, result) => sum + result.time, 0) / successfulLoads.length;
      ...
      logE2EPerformance(
        `Image load performance - Count: ${images.length}, Avg time: ${avgLoadTime}ms`
      );
    }
  });
  ...
  test('should have efficient bundle size', async ({ page }) => {
    ...
    logE2EPerformance(`Bundle sizes - JS: ${totalJSSize} bytes, CSS: ${totalCSSSize} bytes`);
  ...
  test('should handle concurrent requests efficiently', async ({ page }) => {
    ...
    const currentUrl = page.url();
    // SKIP: concurrency checks target the dashboard shell, not login or fund setup flows
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    const actions = [
      ...
      async () => {
        ...
        try {
          ...
          logE2EPerformance('Navigation test failed:', error);
        }
      },
    ];
    ...
    logE2EPerformance(`Concurrent operations time: ${totalTime}ms`);
  });
  ...
  test('should maintain performance under load simulation', async ({ page }) => {
    ...
    const currentUrl = page.url();
    // SKIP: load-simulation checks require the interactive dashboard experience
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    logE2EPerformance(`Load simulation time: ${interactionTime}ms`);
  });
  ...
  test('should cache resources effectively', async ({ page }) => {
    ...
    const currentUrl = page.url();
    // SKIP: cache comparisons are only useful on the intended dashboard route
    test.skip(
      currentUrl.includes('/login') || currentUrl.includes('/fund-setup'),
      'Redirected to different page'
    );
    ...
    logE2EPerformance(`First load: ${firstLoadDuration}ms, Reload: ${reloadDuration}ms`);
