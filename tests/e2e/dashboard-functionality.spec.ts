...
test.describe('Dashboard Functionality', () => {
  ...
  test('should display recent investments section', async () => {
    ...
    if (investmentCount > 0) {
      const firstInvestment = dashboardPage.page
        .locator('[data-testid="investment-item"], .investment-row')
        .first();
  ...
  test('should handle add investment action', async () => {
    ...
    if (addInvestmentVisible) {
      ...
      const modalVisible = await dashboardPage.page
  ...
  test('should handle empty state for new fund', async () => {
    ...
    if (investmentCount === 0) {
      ...
      const emptyStateMessage = dashboardPage.page
        .locator(
          ':has-text("No investments"), :has-text("Get started"), :has-text("Add your first")'
        )
        .first();
      ...
      const hasEmptyStateHandling =
        (await emptyStateMessage.isVisible()) || (await addInvestmentButton.isVisible());
