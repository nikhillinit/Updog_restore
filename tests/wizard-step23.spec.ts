...
test('Step 2 → 3 paints KPIs fast and remains responsive', async ({ page }) => {
  ...
  console.warn(`[Smoke Test] Step 2→3 transition time: ${transitionTime}ms`);
  ...
  if ((await charts.count()) > 0) {
    ...
    console.warn('[Smoke Test] Charts rendered successfully');
  }
});
...
test('Worker can be disabled via feature flag', async ({ page }) => {
  ...
  console.warn('[Smoke Test] Fallback to main thread successful');
});
...
test('Performance marks are recorded correctly', async ({ page }) => {
  ...
  if (measures.length > 0) {
    console.warn(
      `[Smoke Test] Performance measure 'step2->3': ${measures[0].duration.toFixed(2)}ms`
    );
