  ...
  describe('Closed-End Fund Validation', () => {
      ...
      if (!result.success) {
        expect(
          result.error.errors.some(
            (err) => err.message === 'Fund life is required for closed-end funds'
          )
        ).toBe(true);
      }
    });
    ...
    it('should fail validation when investment horizon exceeds fund life', () => {
      ...
      if (!result.success) {
        expect(
          result.error.errors.some(
            (err) => err.message === 'Investment horizon cannot exceed fund life'
          )
        ).toBe(true);
      }
    });
  ...
  describe('European Waterfall Validation', () => {
    it('should validate hurdle and catch-up rates for European waterfall', () => {
      ...
      if (!result.success) {
        expect(
          result.error.errors.some(
            (err) => err.message === 'Catch-up rate must be greater than or equal to hurdle rate'
          )
        ).toBe(true);
      }
    });
