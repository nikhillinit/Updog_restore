...
function getTooltipItemKey(item: TooltipItem, fallback: string): string {
  return (
    dataKeyToString(item.dataKey) ??
    (isNameType(item.name) ? `${item.name}` : fallback)
  )
}
  ...
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const value = !labelKey
        ? getTooltipLabelValue(config, label)
        : itemConfig?.label
...
>(
  (
    ...
    if (
      !rest.active ||
      !isTooltipPayloadList(rest.payload) ||
      rest.payload.length === 0
    ) {
      return null
    }
...
const RechartsDispatcher = React.forwardRef<HTMLDivElement, RechartsDispatcherProps>(
  ({ component, children, ...rest }, ref) => {
    ...
    const Component = componentMap[component] as unknown as React.ElementType
