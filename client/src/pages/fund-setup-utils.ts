...
export type StepKey = 'investment-strategy' | 'exit-recycling' | 'waterfall' | 'not-found';

export const VALID_STEPS = ['2', '3', '4'] as const;
...
/** Missing param (null) -> default '2'. Empty string -> invalid. */
export function normalizeStepParam(value: string | null): ValidStep | null {
  if (value === null) return '2';                // no param -> default
  const v = value.trim();
