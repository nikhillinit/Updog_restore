...
export default function CapitalStructureStep() {
  ...
  const setCapitalStageAllocations = useFundSelector((s) => s.setCapitalStageAllocations);

  // Per-allocation capital plan configs (persisted via fundStore)
  const allocations = useFundSelector((s) => s.capitalPlanAllocations);
  const addCapitalPlanAllocation = useFundSelector((s) => s.addCapitalPlanAllocation);
  const updateCapitalPlanAllocation = useFundSelector((s) => s.updateCapitalPlanAllocation);
  const removeCapitalPlanAllocation = useFundSelector((s) => s.removeCapitalPlanAllocation);
  ...
  const totalAllocationPct = allocations.reduce(
    (sum, alloc) => sum + alloc.capitalAllocationPct,
    0
  );

  const handleAddAllocation = () => {
    const newAllocation: CapitalPlanAllocation = {
    ...
    addCapitalPlanAllocation(newAllocation);
    setEditingAllocation(newAllocation.id);
  };

  const handleUpdateAllocation = (id: string, updates: Partial<CapitalPlanAllocation>) => {
    updateCapitalPlanAllocation(id, updates);
  };

  const handleDeleteAllocation = (id: string) => {
    removeCapitalPlanAllocation(id);
  ...
  const calculateImpliedValues = (allocation: CapitalPlanAllocation) => {
    ...
    if (allocation.followOnStrategy === 'amount' && allocation.followOnAmount) {
      ...
      followOnCapital =
        estimatedDeals * allocation.followOnAmount * (allocation.followOnParticipationPct / 100);
  ...
  const calculateImpliedOwnership = (allocation: CapitalPlanAllocation): number | null => {
    ...
    if (allocation.initialCheckStrategy === 'amount' && allocation.initialCheckAmount) {
      ...
      const postMoneyValuation =
        typicalPostMoneyValuations[
          allocation.entryRound as keyof typeof typicalPostMoneyValuations
        ];
      ...
      <div className="space-y-8">
        ...
        >
          <AllocationSliders
            ...
            onChange={(rows) => {
              setCapitalStageAllocations(rows);
            }}
          />
        </WizardCard>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-3">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleUpdateAllocation(allocation.id, { sectorProfileId: value })
                            }
                          >
                        ...
                        <div className="space-y-3">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleUpdateAllocation(allocation.id, { entryRound: value })
                            }
                          >
                            <SelectTrigger className="h-12">
                              <SelectValue />
                        ...
                        {allocation.initialCheckStrategy === 'amount' ? (
                          <div className="space-y-3">
                            ...
                            <Input
                              type="text"
                              value={
                                allocation.initialCheckAmount
                                  ? formatCurrency(allocation.initialCheckAmount)
                                  : ''
                              }
                              onChange={(e) =>
                                handleUpdateAllocation(allocation.id, {
                                  ...(parseCurrency(e.target.value)
                                    ? { initialCheckAmount: parseCurrency(e.target.value) }
                                    : {}),
                                })
                              }
                              ...
                              onChange={(e) =>
                                handleUpdateAllocation(allocation.id, {
                                  ...(parseFloat(e.target.value)
                                    ? { initialOwnershipPct: parseFloat(e.target.value) }
                                    : {}),
                                })
                              }
                              placeholder="e.g., 10"
                          ...
                          calculations.impliedOwnership !== undefined ? (
                            <p className="text-gray-700">
                              ...
                              <span className="text-xs">
                                (based on typical {allocation.entryRound} valuations)
                              </span>
                            </p>
                          ) : (
                            <p className="text-gray-700">
                          ...
                          <div className="space-y-3">
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="maintain_ownership">
                                  Maintain Ownership (%)
                                </SelectItem>
                              </SelectContent>
                            </Select>
                        ...
                        {allocation.followOnStrategy === 'amount' && (
                          <div className="space-y-3">
                            ...
                            <Input
                              type="text"
                              value={
                                allocation.followOnAmount
                                  ? formatCurrency(allocation.followOnAmount)
                                  : ''
                              }
                              onChange={(e) =>
                                handleUpdateAllocation(allocation.id, {
                                  ...(parseCurrency(e.target.value)
                                    ? { followOnAmount: parseCurrency(e.target.value) }
                                    : {}),
                                })
                              }
