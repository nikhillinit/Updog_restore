...
function toTelemetryDisplayEvent(event: TelemetryEvent, index: number): TelemetryDisplayEvent {
  ...
  const timestampMs = legacyTimestamp ?? (Number.isNaN(parsedTimestamp) ? 0 : parsedTimestamp);
  const category = getLegacyStringField(event, 'category');
  const meta = getLegacyMetaField(event);
  const ok = getLegacyBooleanField(event, 'ok');
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div
            className={`text-2xl font-bold ${errors.length === 0 ? 'text-green-600' : 'text-red-600'}`}
          >
            {errors.length}
          </div>
          <div className="text-gray-600">Total Errors</div>
          <div className="text-sm text-red-600 mt-1">Last hour: {recentErrors.length}</div>
        </div>

        <div className="bg-white rounded-lg shadow p-6">
          <div
            className={`text-2xl font-bold ${errorRate < 1 ? 'text-green-600' : 'text-red-600'}`}
          >
            {errorRate.toFixed(1)}%
          </div>
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        <div className="max-h-96 overflow-y-auto">
          {events.length === 0 ? (
            <div className="p-6 text-center text-gray-500">No telemetry events yet</div>
          ) : (
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            ...
            <span
              className={`font-medium ${
                migrationErrors === 0
                  ? 'text-green-600'
                  : migrationSuccess / Math.max(migrations.length, 1) > 0.99
                    ? 'text-yellow-600'
                    : 'text-red-600'
              }`}
            >
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">{lastHour.length} events in last hour</span>
          </div>
          <div className="flex justify-between">
            <span>Buffer Usage:</span>
            <span className="font-medium">{events.length}/200 events</span>
          </div>
        </div>
      </div>
    </div>
