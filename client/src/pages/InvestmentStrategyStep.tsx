...
import { migrateLegacyPipelineProfiles } from '@/lib/migrate-legacy-pipeline';

// Investment Stage interface (kept for backwards compat with legacy localStorage shape)
...
export default function InvestmentStrategyStep() {
  ...
  const setPipelineProfiles = useFundSelector((s) => s.setPipelineProfiles);

  // Migrate legacy localStorage key on first mount
  useEffect(() => {
    migrateLegacyPipelineProfiles(storeProfiles, setPipelineProfiles);
    // Run once on mount only
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Use store profiles if populated, otherwise fall back to defaults
  const sectorProfiles: SectorProfile[] =
    storeProfiles.length > 0
      ? (storeProfiles as unknown as SectorProfile[])
      : DEFAULT_SECTOR_PROFILES;

  // Wrapper that updates the store (auto-persisted via Zustand persist middleware)
  const setSectorProfiles = useCallback(
    (profiles: SectorProfile[] | ((prev: SectorProfile[]) => SectorProfile[])) => {
      const newProfiles = typeof profiles === 'function' ? profiles(sectorProfiles) : profiles;
      setPipelineProfiles(newProfiles as unknown as PipelineProfile[]);
    },
    [sectorProfiles, setPipelineProfiles]
  );
  ...
  const handleUpdateProfile = (profileId: string, updates: Partial<SectorProfile>) => {
    setSectorProfiles((prev) => prev.map((p) => (p.id === profileId ? { ...p, ...updates } : p)));
  };
  ...
  const handleUpdateStage = (
    profileId: string,
    stageId: string,
    updates: Partial<InvestmentStage>
  ) => {
    setSectorProfiles((prev) =>
      prev.map((p) =>
        p.id === profileId
                    ...
                    <div className="space-y-6">
                            ...
                            >
                              ...
                              <div className="px-3">
                                <Select
                                  ...
                                  onValueChange={(value: string) =>
                                    handleUpdateStage(profile.id, stage.id, { name: value })
                                  }
                                >
                              ...
                              <div className="px-3">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    handleUpdateStage(profile.id, stage.id, {
                              ...
                              <div className="px-3">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    handleUpdateStage(profile.id, stage.id, {
                                      monthsToGraduate: parseInt(e.target.value) || 12,
                                    })
                                  }
                                  className="h-12 text-sm text-center font-medium w-full"
                                  placeholder="18"
                      ...
                      <div className="bg-amber-50 border border-[#E0D8D1] rounded-xl p-4 shadow-sm">
                        ...
                        <ul className="font-poppins text-sm text-[#292929]/80 space-y-1">
                          ...
                          <li>
                            • Time to exit is months after entering that stage, not from initial
                            investment
                          </li>
                          <li>
                            • Don't delete later-stage rounds even if you don't participate - needed
                            for FMV step-ups
                          </li>
                        </ul>
                      </div>
