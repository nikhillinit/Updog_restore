...
class ReservesMetrics {
  ...
  private calculateDivergence(a: unknown, b: unknown): number {
    ...
    if (allocationsA.length !== allocationsB.length) {
      return (
        Math.abs(allocationsA.length - allocationsB.length) /
        Math.max(allocationsA.length, allocationsB.length, 1)
      );
    }
    ...
    allocationsA.forEach((alloc, i: number) => {
      ...
      if (otherAlloc) {
        const plannedCents = alloc['planned_cents']!;
        const otherPlannedCents = otherAlloc['planned_cents']!;
        totalDiff += Math.abs(plannedCents - otherPlannedCents);
        totalAmount += plannedCents;
      }
    });
