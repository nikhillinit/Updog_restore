...
export class ReservesErrorBoundary {
  ...
  private initializeRecoveryStrategies(): void {
      ...
      recover: async (error: Error, context: ErrorContext): Promise<ReservesResult | null> => {
        ...
        try {
          ...
          const simplifiedInput = {
            ...context.input,
            companies:
              context.input.companies
                ?.filter((c) => c.invested_cents > 0 && c.exit_moic_bps > 0)
          ...
          return this.performSimplifiedCalculation(
            simplifiedInput as ReservesInput,
            simplifiedConfig as ReservesConfig
          );
        } catch (recoveryError) {
    ...
    this.addRecoveryStrategy({
      ...
      recover: async (error: Error, context: ErrorContext): Promise<ReservesResult | null> => {
        try {
          ...
          if (cached) {
            const result = JSON.parse(cached) as ReservesResult;
            ...
            return {
              ...
              warnings: [
                ...(result.warnings || []),
                'Using cached fallback data due to calculation error',
              ],
            };
          }
    ...
    this.addRecoveryStrategy({
      ...
      recover: async (error: Error, context: ErrorContext): Promise<ReservesResult | null> => {
        ...
        try {
          const totalInvested =
            context.input.companies?.reduce((sum, c) => sum + c.invested_cents, 0) || 0;
          const reserveAmount = Math.floor(
            (totalInvested * (context.config.reserve_bps || 1500)) / 10000
          );
  ...
  private performSimplifiedCalculation(
    input: ReservesInput,
    config: ReservesConfig
  ): ReservesResult {
    const validCompanies = input.companies.filter(
      (c) => c.invested_cents > 0 && c.exit_moic_bps > 0
    );
  ...
  private isCircuitOpen(context: ErrorContext): boolean {
    const recentErrors = this.errorHistory.filter(
      (entry) =>
        Date.now() - entry.timestamp.getTime() < this.CIRCUIT_BREAKER_WINDOW &&
  ...
  getErrorStats() {
    ...
    const operationCounts = recentErrors.reduce(
      (acc, entry) => {
    ...
    return {
      ...
      circuitStatus: Object.keys(operationCounts).reduce(
        (acc, op) => {
