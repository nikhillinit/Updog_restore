...
const WIZARD_TELEMETRY_ENDPOINT = withApiBase('/api/telemetry/wizard');
  ...
  try {
    ...
    const ok = beaconsSupported
      ? navigator.sendBeacon(
          WIZARD_TELEMETRY_ENDPOINT,
          new Blob([body], { type: 'application/json' })
        )
      : false;

    if (!ok) {
      fetch(WIZARD_TELEMETRY_ENDPOINT, {
        method: 'POST',
        ...
        logger.debug('wizard_telemetry_fetch_failed', {
