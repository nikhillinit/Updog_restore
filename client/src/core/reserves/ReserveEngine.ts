...
import { ReserveCompanyInputSchema, ReserveOutputSchema, ConfidenceLevel } from '@shared/types';
...
export function ReserveEngine(portfolio: unknown[]): ReserveOutput[] {
  ...
  // Validate all inputs
  const validatedPortfolio: ReserveCompanyInput[] = map(
    portfolio,
    (company: unknown, index: number) => {
      try {
...
export function generateReserveSummary(
  ...
  const totalAllocation = reduce(
    allocations,
    (sum: number, item: ReserveOutput) => sum + item.allocation,
    0
  );
  const avgConfidence =
    allocations.length > 0
      ? reduce(allocations, (sum: number, item: ReserveOutput) => sum + item.confidence, 0) /
        allocations.length
      : 0;
